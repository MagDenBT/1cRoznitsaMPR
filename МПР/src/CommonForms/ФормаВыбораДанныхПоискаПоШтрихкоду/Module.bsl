...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТаблицаНоменклатурыДляОстатков.Количество() > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(СкладОстатков) Тогда
			СкладОстатков = РаботаСоШтрихкодамиПереопределяемый.ТекущийСклад();
		КонецЕсли;
...
Процедура ОбновитьОстаткиНоменклатурыВДереве()
	...
	ТаблицаОстатков = РаботаСоШтрихкодамиПереопределяемый.ОстаткиПоСкладу(ТаблицаНоменклатурыДляОстатков.Выгрузить(), СкладОстатков);
