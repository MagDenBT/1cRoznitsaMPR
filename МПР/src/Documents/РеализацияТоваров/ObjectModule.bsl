...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//ММС
	Если ЗначениеЗаполнено(ds_РеализацияПоЗаказам) и ЗаказПокупателя.ds_ЗаказСWildberries Тогда
		ЗаполнитьТЧПоставкаWB();
	КонецЕсли;
	//ММС
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если НЕ Отказ Тогда
		//ММС
		Если ds_РеализацияПоЗаказам и НЕ ЗначениеЗаполнено(ЗаказПокупателя) Тогда
			Заказы = ПолучитьЗаказыОснования();
			Для Каждого Заказ Из Заказы Цикл
				ОбменССайтомРТ.ЗаписатьСостояниеДоставкиЗаказаПокупателя(Заказ, Ссылка);
			КонецЦикла;
		Иначе
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если НЕ Отказ Тогда
		...
		Если ds_РеализацияПоЗаказам и НЕ ЗначениеЗаполнено(ЗаказПокупателя) Тогда
			Заказы = ПолучитьЗаказыОснования();
			Для Каждого Заказ Из Заказы Цикл
				ОбменССайтомРТ.ЗаписатьСостояниеДоставкиЗаказаПокупателя(Заказ, Ссылка, Истина);
			КонецЦикла;
		Иначе
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ds_РеализацияПоЗаказам Тогда
...
Процедура ИнициализироватьДокумент(СтандартнаяОбработка, ДанныеЗаполнения = Неопределено)
	//ММС
	ds_ИнициализироватьДокумент(СтандартнаяОбработка, ДанныеЗаполнения);
	Возврат;
	//ММС
...
#Область КастомизацияDS

Процедура ЗаполнитьТЧПоставкаWB()
		
	тзТовары = Товары.ВыгрузитьКолонки("Номенклатура,Количество");
	тзТовары.Свернуть("Номенклатура");
	ds_ПоставкаWB.Очистить();
	ds_ПоставкаWB.Загрузить(тзТовары);

КонецПроцедуры

Процедура ds_ИнициализироватьДокумент(СтандартнаяОбработка, ДанныеЗаполнения = Неопределено)
	...
	Если Не ДанныеЗаполнения = Неопределено Тогда
		...
		Если ТипДанныхЗаполнения = Тип("Структура")  И ДанныеЗаполнения.Свойство("ДокументОснование") //ММС
	...
	Если ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("ДокументОснование")
					 И ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("Массив") Тогда //ММС
	...
	Если ЗначениеЗаполнено(ЗаказПокупателя) Тогда
		...
		РозничныеПродажиСервер.ЗаполнитьПоОстаткамЗаказа(ЭтотОбъект, ЗаказПокупателя);
		ds_РеализацияПоЗаказам = Истина;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(МассивЗаказов) Тогда
		ds_РозничныеПродажиСервер.ЗаполнитьПоОстаткамЗаказа(ЭтотОбъект, МассивЗаказов);
		ds_РеализацияПоЗаказам = Истина;
	КонецЕсли;
...
Функция ПолучитьЗаказыОснования()
	тзДанные = Товары.Выгрузить();
	тзДанные.Свернуть("ЗаказПокупателя");
	Возврат тзДанные.ВыгрузитьКолонку("ЗаказПокупателя");	
КонецФункции
#КонецОбласти
