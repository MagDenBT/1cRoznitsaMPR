...
Функция ДобавитьНайденныеПозицииТоваровСервер(СтруктураПараметров, ДополнительныеПараметры = Неопределено)
	...
	Если Не ДанныеНоменклатуры.ЭтоТабачнаяПродукция И Не ДанныеНоменклатуры.ЭтоОбувнаяПродукция Тогда 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПродажаПоЗаказу = Объект.ds_РеализацияПоЗаказам;
	//ММС
	...
	Если Элементы.Найти("ТоварыФискальныеДанные") <> Неопределено Тогда
		Элементы.ТоварыФискальныеДанные.Видимость = ИспользоватьКомиссионнуюТорговлю;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	//ММС
	УправлениеИнтеграциейWildberries();	
	//ММС
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.ЗагрузкаДанныхИзВнешнихФайлов.Форма.Форма" Тогда
		ds_ПолучитьЗагруженныеТоварыИзХранилища(ВыбранноеЗначение);
	//}	
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//ММС
		Если Объект.ds_ЭтоПоставкаWB Тогда
			ПроверитьТЧПоставкаWB(Отказ, ПараметрыЗаписи.РежимЗаписи);
		КонецЕсли;
	//ММС
...
//	КонецЕсли;
	Если Объект.ds_РеализацияПоЗаказам Тогда
			Для Каждого заказ Из СписокЗаказов Цикл
				Оповестить("ЗаказПокупателя_Состояния", заказ);
			КонецЦикла;
	КонецЕсли;
	//ММС
...
Процедура ОткрытьЗаказПокупателя(Команда)
	//ММС
	ds_ОткрытьЗаказПокупателя(Команда);
	Возврат;
	//ММС
...
Процедура ОчиститьЗаказПокупателя(Команда)
	...
	Объект.ds_РеализацияПоЗаказам = Ложь;
	УправлениеЭлементамиЗаказПокупателя();
...
Процедура РассчитатьСкидкиНаценкиКлиент()
	...
	//{ds-06.08.2021
	Если Объект.ds_БезАвтоматическихСкидок Тогда
		ОтменитьСкидки();
	КонецЕсли;	
	//}	
...
Процедура ВыборЗаказаПокупателяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	//ММС
	ds_ВыборЗаказаПокупателяЗавершение(Результат, ДополнительныеПараметры);
	Возврат;
	//ММС
...
&НаКлиенте
Процедура ds_ВыборЗаказаПокупателяЗавершение(Результат, ДополнительныеПараметры)
	...
	Объект.ds_РеализацияПоЗаказам = Истина;
	Если ТипЗнч(Результат) = Тип ("Массив") Тогда
		СписокЗаказов.ЗагрузитьЗначения(Результат);
	Иначе
		Объект.ЗаказПокупателя = Результат;
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаказуПокупателя()
	...
	УправлениеИнтеграциейWildberries();
	Если Объект.ds_ЭтоПоставкаWB Тогда
		ds_ЗаполнитьПоставкуWBПоТоварам();
	КонецЕсли;
	//ММС
...
Процедура ЗаполнитьПоЗаказуПокупателяНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ЗаказПокупателя) Тогда
		ds_РозничныеПродажиСервер.ЗаполнитьПоОстаткамЗаказа(Объект, Объект.ЗаказПокупателя);
	Иначе
		ds_РозничныеПродажиСервер.ЗаполнитьПоОстаткамЗаказа(Объект, СписокЗаказов.ВыгрузитьЗначения());
	КонецЕсли;
	//ММС
...
Процедура УправлениеЭлементамиЗаказПокупателя()
	...
	//ММС
	ds_УправлениеЭлементамиЗаказПокупателя();
	Возврат;
	//ММС
...
Функция ДоступныеОсобенностиУчета()
	...
	МаркируемаяПродукция = ИнтеграцияИСКлиентСервер.ВидыПродукцииИСМП(Ложь, Истина);
...
#Область ds_ОбработчикиКомандФормы

&НаКлиенте
Процедура ds_ЗагрузитьИзВнешнегоФайла(Команда)
...
&НаСервере
Процедура ds_ПолучитьЗагруженныеТоварыИзХранилища(РезультатВыбора)
	...
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		...
		ОбработкаТабличнойЧастиТоварыСервер.ВыделитьАгентскоеВознаграждение(Объект, ЭтотОбъект, СтруктураДействий, СтрокаТЧТовары, КэшированныеЗначения);
	КонецЦикла;
...
&НаСервере
Процедура ПроверитьТЧПоставкаWB(Отказ, РежимЗаписи)
	
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда	
		
		тзТовары = Объект.Товары.Выгрузить(,"Номенклатура, Количество" );
		тзТовары.Свернуть("Номенклатура","Количество");
		
		тзПоставкаWB = Объект.ds_ПоставкаWB.Выгрузить(,"Номенклатура, Количество" );
		тзПоставкаWB.Свернуть("Номенклатура","Количество");
		
		Несоответствующие = новый ТаблицаЗначений();
		Несоответствующие.Колонки.Добавить("Номенклатура",новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		Несоответствующие.Колонки.Добавить("Количество",новый ОписаниеТипов("Число"));
		
		//Сверим обе ТЧ. "+" - излишек в Поставке, "-" - недостача в Поставке  
		
		//По тч Поставке
		Для Каждого стр Из тзТовары Цикл
			НайденВПоставке = 	тзПоставкаWB.Найти(стр.Номенклатура);
			Если ЗначениеЗаполнено(НайденВПоставке) Тогда
				Если НЕ НайденВПоставке.Количество = стр.Количество  Тогда
					НоваяСтрока =  Несоответствующие.Добавить();
					НоваяСтрока.Номенклатура = стр.Номенклатура;
					НоваяСтрока.Количество = НайденВПоставке.Количество - стр.Количество ;
				КонецЕсли;
			иначе
					НоваяСтрока =  Несоответствующие.Добавить();
					НоваяСтрока.Номенклатура = стр.Номенклатура;
					НоваяСтрока.Количество = 0 - стр.Количество ;
			КонецЕсли;
		КонецЦикла;
		
		//По тч Товары
		Для Каждого стр Из тзПоставкаWB Цикл
			НайденВТоварах = 	тзТовары.Найти(стр.Номенклатура);
			Если НЕ ЗначениеЗаполнено(НайденВТоварах) Тогда
					НоваяСтрока =  Несоответствующие.Добавить();
					НоваяСтрока.Номенклатура = стр.Номенклатура;
					НоваяСтрока.Количество = 0 - стр.Количество ;
			КонецЕсли;
		...
		Если Несоответствующие.Количество()>0 Тогда
			Отказ = Истина;
					Сообщение = новый СообщениеПользователю();
					Сообщение.ПутьКДанным = Объект.ds_ПоставкаWB;
					Для Каждого ошибка Из Несоответствующие Цикл
					Сообщение.Текст  = Строка(Ошибка.Номенклатура) + ?(ошибка.Количество<0," меньше на " + Строка(ошибка.Количество * -1), " больше на " + Строка(ошибка.Количество));
						Сообщение.Сообщить(); 
					КонецЦикла;
			Сообщение.Текст = "Исправьте несоответствие между таблицей Товары и Поставка Wildberries";
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура УправлениеИнтеграциейWildberries()
	
	Если НЕ Объект.ds_ЭтоПоставкаWB Тогда
		Элементы.ГруппаПоставкаWB.Видимость = Ложь;
		Элементы.Товары.ПодчиненныеЭлементы.ТоварыСтикерWBЗагружен.Видимость = Ложь;
	Иначе
		Элементы.ГруппаПоставкаWB.Видимость = Истина;;
		Элементы.Товары.ПодчиненныеЭлементы.ТоварыСтикерWBЗагружен.Видимость = Истина;
		ОтметкаСтикеры();
	КонецЕсли;
...
&НаСервере
Процедура ОтметкаСтикеры()
		Запрос = ПолучитьЗапросСтикеры();
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
		стрТЧ =	Объект.Товары.НайтиСтроки(новый Структура("НомерСтроки",Выборка.НомерСтроки ));
		стрТЧ[0].СтикерWBЗагружен = Выборка.СтикерЕсть;
		КонецЦикла;
КонецПроцедуры

&НаСервере
Функция ПолучитьЗапросСтикеры()
		...
		|	ВТ.НомерСтроки,
		|	ВЫБОР
		|		КОГДА ЗаказПокупателяТовары.ds_ШтрикходСтикера = """"
		|			Тогда Ложь
		|		ИНАЧЕ Истина
		|	КОНЕЦ КАК СтикерЕсть
		|ИЗ
		|	ВТ КАК ВТ
		...
		Запрос.УстановитьПараметр("ТоварыРеализации", Объект.Товары.Выгрузить(,"ЗаказПокупателя,КодСтроки,НомерСтроки" ) );
	Возврат Запрос;
	
КонецФункции

&НаСервере
Процедура ds_ЗаполнитьПоставкуWBПоТоварам()
	тзТовары = Объект.Товары.Выгрузить(,"Номенклатура,Количество");
	тзТовары.Свернуть("Номенклатура","Количество");
	Объект.ds_ПоставкаWB.Очистить();
	Объект.ds_ПоставкаWB.Загрузить(тзТовары);
КонецПроцедуры

&НаКлиенте
Процедура ds_ПерезаполнитьПоставку(Команда)
	ds_ЗаполнитьПоставкуWBПоТоварам();
КонецПроцедуры

&НаКлиенте
Процедура ds_ЭтоПоставкаWBПриИзменении(Элемент)
	Если Не Объект.ds_ЭтоПоставкаWB Тогда
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить("Продолжить", НСтр("ru = 'Очистить Поставку WB'"));
		ТекстВопроса = НСтр("ru='Список ""Поставка WB"" будет очищен. Продолжить?'");
		СписокКнопок.Добавить("Отмена", НСтр("ru = 'Отмена'"));
		ПоказатьВопрос(Новый ОписаниеОповещения("ds_ЭтоПоставкаWBПриИзмененииЗавершение", ЭтотОбъект), ТекстВопроса,
			СписокКнопок);
		Возврат;
	Иначе
		ds_ЗаполнитьПоставкуWBПоТоварам();
...
&НаКлиенте
Процедура ds_ЭтоПоставкаWBПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Ответ = РезультатВопроса;
	Если Ответ = "Отмена" Тогда
		Объект.ds_ЭтоПоставкаWB = Истина;
		Возврат;
	Иначе
		Объект.ds_ПоставкаWB.Очистить();
	КонецЕсли;
...
&НаКлиенте
Процедура ds_УправлениеЭлементамиЗаказПокупателя()
	...
	ПараметрыВыбораДокументРасчетов = Новый Массив;
	Если ПродажаПоЗаказу Тогда
		ПараметрыВыбораДокументРасчетов.Добавить(Новый ПараметрВыбора("Отбор.ЗаказПокупателя", СписокЗаказов.ВыгрузитьЗначения()));
	КонецЕсли;
	Элементы.ДокументРасчета.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораДокументРасчетов);
...
Процедура ОбновитьИнформациюПоЗаказамВФорме()
	...
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		Если ЗначениеЗаполнено(ТекСтрока.ЗаказПокупателя) И СписокЗаказов.НайтиПоЗначению(ТекСтрока.ЗаказПокупателя)
			= Неопределено Тогда
			СписокЗаказов.Добавить(ТекСтрока.ЗаказПокупателя);
		КонецЕсли;
	КонецЦикла;
	КонецЕсли;

	Если СписокЗаказов.Количество() = 1 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(ЭтаФорма, "Объект.ЗаказПокупателя",
			СписокЗаказов[0].Значение);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(ЭтаФорма, "Объект.ЗаказПокупателя", Неопределено);
	КонецЕсли;

	Если СписокЗаказов.Количество() > 1 Тогда
		НадписьВсегоЗаказов = НСтр("ru = 'Всего заказов'");
		Элементы.ОткрытьЗаказПокупателя.Заголовок =  НадписьВсегоЗаказов + ": " + СписокЗаказов.Количество();
	Иначе
		Элементы.ОткрытьЗаказПокупателя.Заголовок = Строка(Объект.ЗаказПокупателя);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ds_ОткрытьЗаказПокупателя(Команда)
	Если СписокЗаказов.Количество() = 1 Тогда
		ПоказатьЗначение(, Объект.ЗаказПокупателя);
	Иначе
		СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура ds_ПечатьЭтикеток(Команда)
	...
	Если Модифицированность  Тогда
		ПоказатьПредупреждение(, "Сохраните изменения в документе");
		Возврат;
	КонецЕсли;
	
	СтруктураПараметры = ds_СтруктураДанныхЦенниковИЭтикеток();
		
	МассивПечатныхФорм = ds_ПечатьЭтикетокНаСервере(СтруктураПараметры);
 	УправлениеПечатьюКлиент.ПечатьДокументов(МассивПечатныхФорм, , ЭтаФорма);
	
	//ОткрытьФорму("ВнешняяОбработка." + ИмяВнОбработки + ".Форма.ФормаПечатиЭтикеток", СтруктураПараметры, , Истина);
		
КонецПроцедуры

&НаСервере
Функция ds_ПечатьЭтикетокНаСервере(СтруктураПараметры)
	...
	Если Отладка Тогда
		...
		ВнОбработка = ВнешниеОбработки.Создать("C:\EDT projects\MPR\ВнешниеОбработкиОтчеты\bin\ОбменВБ.epf", Ложь,
			ОписаниеЗащиты);
	Иначе
		ВнОбработкаОб = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоНаименованию(
			"Рабочее место Wildberries").Ссылка.ХранилищеОбработки.Получить();
		АдресВоВнутреннемХранилище = ПоместитьВоВременноеХранилище(ВнОбработкаОб);
		ИмяОбработки = ВнешниеОбработки.Подключить(АдресВоВнутреннемХранилище, , Ложь);
		ВнОбработка = ВнешниеОбработки.Создать(ИмяОбработки);
	КонецЕсли;

	КоллекцияПечатныхФорм = УправлениеПечатью.ПодготовитьКоллекциюПечатныхФорм("ПечатьЭтикетокВБ");

	ВнОбработка.Печать(КоллекцияПечатныхФорм, СтруктураПараметры);
...
&НаСервере
Функция ds_СтруктураДанныхЦенниковИЭтикеток() 
	...
	|ГДЕ
	|	РеализацияТоваровТовары.Ссылка = &Реализация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТоварыРеализации.Организация,
	|	ТоварыРеализации.Характеристика,
	|	ЗаказПокупателяТовары.ds_ШтрикходСтикера КАК Штрихкод,
	|	ЗаказПокупателяТовары.ds_idСтикера КАК ШтрихкодЧитабельный,
	|	1 КАК Количество,
	|	ТоварыРеализации.НомерСтроки
	|ПОМЕСТИТЬ ВТЗаготовка
	|ИЗ
	|	ТоварыРеализации КАК ТоварыРеализации
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
	|		ПО ТоварыРеализации.Номенклатура = ЗаказПокупателяТовары.Номенклатура
	|		И ТоварыРеализации.ЗаказПокупателя = ЗаказПокупателяТовары.Ссылка
	|		И ТоварыРеализации.КодСтроки = ЗаказПокупателяТовары.КодСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТЗаготовка.Номенклатура,
	|	ВТЗаготовка.Упаковка,
	|	ВТЗаготовка.Организация,
	|	ВТЗаготовка.Характеристика,
	|	ВТЗаготовка.Штрихкод,
	|	ВТЗаготовка.Количество,
	|	ВТЗаготовка.ШтрихкодЧитабельный
	|ИЗ
	|	ВТЗаготовка КАК ВТЗаготовка
	|ГДЕ
	|	НЕ ВТЗаготовка.Штрихкод ЕСТЬ NULL
	|	И НЕ ВТЗаготовка.ШтрихкодЧитабельный ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВТЗаготовка.НомерСтроки";
	...
	СтруктураРезультат.Вставить("Товары", тзТовары);
	
	
	
	Возврат новый Структура("АдресВХранилище", ПоместитьВоВременноеХранилище(СтруктураРезультат,УникальныйИдентификатор));
