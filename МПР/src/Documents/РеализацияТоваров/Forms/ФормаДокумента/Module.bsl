...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//ММС
		Если ПродажаПоЗаказу и Объект.ЗаказПокупателя.ds_ЗаказСWildberries Тогда
...
Процедура ЗаполнитьПоЗаказуПокупателя()
	...
	//ММС
	УправлениеВкладкойПоставкаWB();
	Если ЗначениеЗаполнено(Объект.ЗаказПокупателя.ds_ЗаказСWildberries) Тогда
...
Процедура ПроверитьТЧПоставкаWB(Отказ, РежимЗаписи)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда	
		...
		тзПоставкаWB.Свернуть("Номенклатура");
		
		Несоответствующие = тзПоставкаWB.Скопировать();
		Несоответствующие.Очистить();
		...
		Если Несоответствующие.Количество()>0 Тогда
			Отказ = Истина;
					...
					Сообщение.Текст  = Ошибка.Номенклатура + ?(ошибка.Количество<0," меньше на " + Строка(ошибка.Количество * -1) , " больше на " + Строка(ошибка.Количество));
						Сообщение.Сообщить(); 
					КонецЦикла;
...
&НаКлиенте
Процедура УправлениеВкладкойПоставкаWB()
	
	Если НЕ ПродажаПоЗаказу или НЕ ЗначениеЗаполнено(Объект.ЗаказПокупателя.ds_ЗаказСWildberries) Тогда
...
Процедура ЗаполнитьПоставкуWBПоЗаказу()
	тзТовары = Объект.Товары.ВыгрузитьКолонки("Номенклатура,Количество");
	тзТовары.Свернуть("Номенклатура");
