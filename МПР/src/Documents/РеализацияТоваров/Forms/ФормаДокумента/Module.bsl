...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//ММС
		Если ПоЗаказуСWildberries() Тогда
...
Процедура УправлениеИнтеграциейWildberries()
	...
	Если НЕ ПоЗаказуСWildberries() Тогда
...
&НаСервере
Процедура ЗаполнитьПоставкуWBПоЗаказу()
	тзТовары = Объект.Товары.Выгрузить(,"Номенклатура,Количество");
	тзТовары.Свернуть("Номенклатура","Количество");
...
&НаСервере
Функция ПоЗаказуСWildberries()
	Результат = Ложь;
	Если ЗначениеЗаполнено(Объект.ЗаказПокупателя) Тогда
		Результат = Объект.ЗаказПокупателя.ds_ЗаказСWildberries;
	КонецЕсли;
	Возврат Результат;
КонецФункции
