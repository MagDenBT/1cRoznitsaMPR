...
Функция СформироватьПечатнуюФормуКО2(МассивОбъектов, ОбъектыПечати)
	...
	//{ds-28.03.2021
	|	,ЕСТЬNULL(РасходныйКассовыйОрдер.КассаККМ.Магазин.ds_НеВыводитьПрефиксВКассовыхДокументах,
	|		ЕСТЬNULL(РасходныйКассовыйОрдер.Касса.Магазин.ds_НеВыводитьПрефиксВКассовыхДокументах, ЛОЖЬ)) КАК НеВыводитьПрефикс
	//}
	|ИЗ
	|	Документ.РасходныйКассовыйОрдер КАК РасходныйКассовыйОрдер
	|
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		//{ds-28.03.2021
		Если ДанныеПечати.НеВыводитьПрефикс Тогда 
			ОбластьМакета.Параметры.НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.Номер, Истина, Истина);
		Иначе
		//}	
		...
		Если СтрДлина(ДанныеПечати.Основание) <= ДлинаПрописныхПолей Тогда
			Основание            = ДанныеПечати.Основание;
			ОснованиеПродолжение = "";
		Иначе
			ПозицияРазделитиля = СтрНайти(ДанныеПечати.Основание, " ", НаправлениеПоиска.СКонца, ДлинаПрописныхПолей);
			
			Основание = Лев(ДанныеПечати.Основание, ПозицияРазделитиля);
			ОснованиеПродолжение = Сред(ДанныеПечати.Основание, ПозицияРазделитиля + 1);
		КонецЕсли;
		ОбластьМакета.Параметры.Основание             = Основание;
		ОбластьМакета.Параметры.ОснованиеПродолжение  = ОснованиеПродолжение;
		
		ОбластьМакета.Параметры.Приложение            = ДанныеПечати.Приложение;
		ОбластьМакета.Параметры.ФИОПолучателя         = ДанныеПечати.Выдать;
		
		Если СтрДлина(ДанныеПечати.ПоДокументу) <= ДлинаПрописныхПолей Тогда
			ПоДокументу            = ДанныеПечати.ПоДокументу;
			ПоДокументуПродолжение = "";
		Иначе
			ПозицияРазделитиля = СтрНайти(ДанныеПечати.ПоДокументу, " ", НаправлениеПоиска.СКонца, ДлинаПрописныхПолей);
			
			ПоДокументу = Лев(ДанныеПечати.ПоДокументу, ПозицияРазделитиля);
			ПоДокументуПродолжение = Сред(ДанныеПечати.ПоДокументу, ПозицияРазделитиля + 1);
		КонецЕсли;
		ОбластьМакета.Параметры.РеквизитыДокументаУдостоверяющегоЛичность            = ПоДокументу;
		ОбластьМакета.Параметры.РеквизитыДокументаУдостоверяющегоЛичностьПродолжение = ПоДокументуПродолжение;
