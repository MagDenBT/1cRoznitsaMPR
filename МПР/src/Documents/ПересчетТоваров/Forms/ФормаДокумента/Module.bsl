...
//{ds-28.04.2021
&НаКлиенте
Процедура ds_ПодборДокументыОснования(Команда)

	П = Новый Структура;
	П.Вставить("ЗакрыватьПриВыборе", Ложь);
	П.Вставить("МножественныйВыбор", Истина);
	П.Вставить("Отбор", Новый Структура("Ссылка", ds_СписокДоступныхДокументовОснований()));
	
	ОткрытьФорму("Документ.ПересчетТоваров.ФормаВыбора", П, Элементы.ds_ДокументыОснования);
	
КонецПроцедуры

&НаСервере
Функция ds_СписокДоступныхДокументовОснований()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПересчетТоваров.ds_ДокументыОснования КАК ПересчетТоваровds_ДокументыОснования
	|		ПО ПересчетТоваровds_ДокументыОснования.Ссылка = ПересчетТоваров.Ссылка
	|		И ПересчетТоваровds_ДокументыОснования.ДокументОснование = ПересчетТоваров.Ссылка
	|ГДЕ
	|	НЕ ПересчетТоваров.ПометкаУдаления
	|	И ПересчетТоваров.Магазин = &Магазин
	|	И ПересчетТоваров.Ссылка НЕ В (&Основания)
	|	И ПересчетТоваровds_ДокументыОснования.Ссылка ЕСТЬ NULL";
	Запрос.УстановитьПараметр("Магазин", Объект.Магазин);
	
	Основания = Объект.ds_ДокументыОснования.Выгрузить().ВыгрузитьКолонку("ДокументОснование");
	Основания.Добавить(Объект.Ссылка);
...
&НаКлиенте
Процедура ds_ДокументыОснованияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Массив") Тогда 
		Для Каждого Стр Из ВыбранноеЗначение Цикл 
			ds_ОбработкаВыбораДокументыОснованияПодборНаСервере(Стр);
...
&НаСервере 
Процедура ds_ОбработкаВыбораДокументыОснованияПодборНаСервере(ДокументОснование)
	
	Если Не Объект.ds_ДокументыОснования.НайтиСтроки(Новый Структура("ДокументОснование", ДокументОснование)).Количество() Тогда 
		НовСтр = Объект.ds_ДокументыОснования.Добавить();
...
&НаКлиенте
Процедура ds_ЗаполнитьПоОснованиям(Команда)
	
	ds_ЗаполнитьПоОснованиямНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ds_ЗаполнитьПоОснованиямНаСервере()
	...
	|	СУММА(ВложенныйЗапрос.КоличествоУпаковокФакт) КАК КоличествоФакт,
	...
	|		0,
	|		СУММА(ПересчетТоваровТовары.КоличествоУпаковокФакт)
	|	ИЗ
	|		Документ.ПересчетТоваров.Товары КАК ПересчетТоваровТовары
	|	ГДЕ
	|		ПересчетТоваровТовары.Ссылка В(&ДокументыОснования)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ПересчетТоваровТовары.Номенклатура,
	|		ПересчетТоваровТовары.Упаковка) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Номенклатура,
	|	ВложенныйЗапрос.Упаковка";
	Запрос.УстановитьПараметр("Товары", Объект.Товары.Выгрузить());
	Запрос.УстановитьПараметр("ДокументыОснования", Объект.ds_ДокументыОснования.Выгрузить().ВыгрузитьКолонку("ДокументОснование"));
