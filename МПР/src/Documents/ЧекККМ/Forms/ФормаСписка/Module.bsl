...
Функция ДанныеПоискаПоШтрихкодуСервер(Штрихкод)
	...
	ПодключаемоеОборудованиеРТ.ДанныеПоискаЧекаПоДаннымQRКода(Штрихкод, ДанныеПоискаПоШтрихкоду);
...
Процедура ВыполнитьВнесениеДенегВКассуККМКлиент()
	...
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Будет выполнено внесение денежных средств в кассу %1 на сумму %2 руб. по документу %3.
				|Продолжить?'"),
...
Процедура ВыполнитьОперациюНаЭквайринговомТерминале(Операция, ЭквайринговыйТерминал)
	...
	Если Операция = "СверкаИтогов" Тогда
		...
		ОборудованиеПлатежныеСистемыКлиент.НачатьВыполнениеСверкиИтоговНаЭквайринговомТерминале(ОписаниеОповещения, УникальныйИдентификатор, ЭквайринговыйТерминал,,,ПечатающееУстройство);
	ИначеЕсли Операция = "ПечатьПоследнегоСлипЧека" Тогда
		Если Не ПустаяСтрока(глПодключаемоеОборудование.ПоследнийСлипЧек) Тогда
			...
			ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьПечатьТекста(Оповещение, УникальныйИдентификатор, ПечатающееУстройство, ПараметрыОперации);
		Иначе
