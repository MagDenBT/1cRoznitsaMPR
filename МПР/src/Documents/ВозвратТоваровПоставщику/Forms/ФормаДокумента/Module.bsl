...
&НаКлиенте
Процедура ds_ТоварыДокументОснованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураПараметров.Вставить("ИндексТекущейСтроки", ТекущаяСтрока.ПолучитьИдентификатор());
		
	СписокДокументов = ds_ПолучитьСписокДокументовСодержащихНоменклатуру(СтруктураПараметров);
	
	Если СписокДокументов.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Документы закупки данного товара не найдены.'"));
		Возврат;
	ИначеЕсли СписокДокументов.Количество() = 1 Тогда
		Выбор = СписокДокументов[0];
		ds_ЗавершитьВыборДокументаЗакупки(Выбор.Значение, ТекущаяСтрока)
	Иначе
		ТекстПодсказки = НСтр("ru = 'Выбор документа закупки:'");
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ИндексТекущейСтроки", ТекущаяСтрока.ПолучитьИдентификатор());
		
		ОбработчикОповещения = Новый ОписаниеОповещения("ds_ОповещениеВыборЭлементаДокументЗакупки", ЭтотОбъект, ДополнительныеПараметры);
...
&НаСервереБезКонтекста
Функция ds_ПолучитьСписокДокументовСодержащихНоменклатуру(СтруктураПараметров)
	
	Результат = Новый СписокЗначений;
	
	СписокДокументов = ds_СписокДокументовСодержащихНоменклатуру(СтруктураПараметров);
		
	Результат.ЗагрузитьЗначения(СписокДокументов.ВыгрузитьКолонку("ДокументЗакупки"));
...
&НаСервереБезКонтекста
Функция ds_СписокДокументовСодержащихНоменклатуру(СтруктураПараметров)

	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПоступлениеТоваровТовары.Ссылка КАК ДокументЗакупки,
...
&НаКлиенте
Процедура ds_ЗавершитьВыборДокументаЗакупки(ВыбранныйДокумент, ТекущаяСтрока)
	
	Если ЗначениеЗаполнено(ВыбранныйДокумент)  Тогда	
		РезультатОтбораСтрок = ds_СтрокиДокументаЗакупкиНаКлиента(ВыбранныйДокумент, ТекущаяСтрока.Номенклатура, УникальныйИдентификатор);
		...
		Если ЗначениеЗаполнено(РезультатОтбораСтрок)  Тогда
			...
			ОбработчикОповещения = Новый ОписаниеОповещения("ds_ОповещениеОткрытьФормуВыбораСтрокиДокумента", ЭтотОбъект, ДополнительныеПараметры);
			Режим = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
			ОткрытьФорму("Документ.ВозвратТоваровПоставщику.Форма.ds_ФормаВыбораСтрокиДокумента", ПараметрыФормы,,,,, ОбработчикОповещения, Режим);
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ds_СтрокиДокументаЗакупкиНаКлиента(Документ, Номенклатура, УникальныйИдентификатор)
	...
	|	ПоступлениеТоваровТовары.Ссылка КАК ДокументЗакупки,
	|	ПоступлениеТоваровТовары.Количество КАК КоличествоВДокументеЗакупки,
	|	ПоступлениеТоваровТовары.Сумма,
	|	ПоступлениеТоваровТовары.СтавкаНДС,
	|	ВЫБОР
	|		КОГДА ПоступлениеТоваровТовары.Количество = 0
	|			ТОГДА ПоступлениеТоваровТовары.Цена
	|		ИНАЧЕ ПоступлениеТоваровТовары.Сумма / ПоступлениеТоваровТовары.Количество
	|	КОНЕЦ КАК Цена
	|ИЗ
	|	Документ.ПоступлениеТоваров.Товары КАК ПоступлениеТоваровТовары
	|ГДЕ
...
&НаКлиенте
Процедура ds_ОповещениеВыборЭлементаДокументЗакупки(Выбор, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ds_ОповещениеОткрытьФормуВыбораСтрокиДокумента(РезультатОткрытияФормы, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатОткрытияФормы = Неопределено Тогда
		...
		ТекущаяСтрока.ds_ДокументОснование = РезультатОткрытияФормы.ДокументЗакупки;
