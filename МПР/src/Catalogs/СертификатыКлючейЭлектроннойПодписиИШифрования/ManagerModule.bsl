...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	СертификатыКлючейЭлектроннойПодписиИШифрования.ПользовательОповещенОСрокеДействия
	...
	Если ЭлектроннаяПодпись.ОбщиеНастройки().ЗаявлениеНаВыпускСертификатаДоступно Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеЗаявления",
			"СертификатыКлючейЭлектроннойПодписиИШифрования.СостояниеЗаявления <> &ПустаяСсылка");
...
Процедура ОбработатьЗаявленияИОповещенияСертификатов(Выборка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если СертификатОбъект.ПользовательОповещенОСрокеДействия Тогда
				...
				КонецЕсли;
				СертификатОбъект.ПользовательОповещенОСрокеДействия = Ложь;
			КонецЕсли;
			
			// Перенос данных заявлений в регистр сведений.
			Если ЗаявлениеНаВыпускСертификатаДоступно
			   И ЗначениеЗаполнено(СертификатОбъект.СостояниеЗаявления) Тогда
