...
Процедура ОбработкаРазблокированияРеквизитовЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИнтеграцияИСРТКлиентСервер.ЗаполнитьДанныеНастройкиПараметровНоменклатурыИСМП(ЭтотОбъект, Результат);
...
Процедура КодТНВЭДПриИзменении(Элемент)
	...
	Иначе
		Объект.ЕдиницаИзмеренияТНВЭД = ПредопределенноеЗначение("Справочник.БазовыеЕдиницыИзмерения.ПустаяСсылка");
	КонецЕсли;
...
Процедура УстановитьДоступностьРеквизитовНаСервере()
	...
	Если ЭтоТовар И ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП") Тогда
		...
		ЭтоМаркируемаяПродукцияИСМП = ИнтеграцияИСКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции, Ложь);
	КонецЕсли;
	...
	Если Объект.ОсобенностьУчета <> Перечисления.ОсобенностиУчетаНоменклатуры.БезОсобенностейУчета Тогда
		...
		Если Объект.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукция
			И Объект.ВидНоменклатуры.ПродаетсяВРозлив Тогда
...
Процедура ВидимостьПрослеживаемыйТовар()
	...
	ПрослеживаемыйТовар = Объект.КодТНВЭД.ПрослеживаемыйТовар = Истина;
