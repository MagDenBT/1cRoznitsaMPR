...
//{ds-07.03.2021
&НаКлиенте
Процедура ds_ЗаполнитьКатегории(Команда)
	
	ds_ЗаполнитьКатегорииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ds_ЗаполнитьКатегорииНаСервере()

	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ds_КатегорииНоменклатуры.Ссылка КАК Категория
	|ИЗ
	|	Справочник.ds_КатегорииНоменклатуры КАК ds_КатегорииНоменклатуры
	|ГДЕ
	|	ds_КатегорииНоменклатуры.Действует
	|
	|УПОРЯДОЧИТЬ ПО
	|	ds_КатегорииНоменклатуры.Наименование";
	
	Объект.ds_КатегорииИсключение.Загрузить(Запрос.Выполнить().Выгрузить());
