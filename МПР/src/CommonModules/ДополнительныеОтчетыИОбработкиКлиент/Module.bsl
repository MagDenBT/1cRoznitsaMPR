...
Процедура ВыполнитьОткрытиеФормыОбработки(ВыполняемаяКоманда, Форма, ОбъектыНазначения) Экспорт
	...
	#Иначе
		...
		Если ВыполняемаяКоманда.ЭтоОтчет Тогда
			...
			ОткрытьФорму("ВнешнийОтчет." + ИмяОбработки + ".Форма", ПараметрыОбработки, Форма, Новый УникальныйИдентификатор());
			//}
		Иначе
			//{ds-20.10.2021
			//ОткрытьФорму("ВнешняяОбработка." + ИмяОбработки + ".Форма", ПараметрыОбработки, Форма);
			ОткрытьФорму("ВнешняяОбработка." + ИмяОбработки + ".Форма", ПараметрыОбработки, Форма, Новый УникальныйИдентификатор());
			//}
		КонецЕсли;
	#КонецЕсли
КонецПроцедуры
