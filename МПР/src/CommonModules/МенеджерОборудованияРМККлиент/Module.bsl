...
Процедура ОбновитьДанныеНаДисплееПокупателя(ИсходныеДанные, ОбъектВладелец, УникальныйИдентификатор) Экспорт
	...
	Иначе
		...
		Если НЕ ОтображатьТолькоИтоги Тогда
			...
			ДанныеДляОтображения.ТоварЦена = ОбщегоНазначенияРМККлиентСервер.ФорматСумм(ТекущаяСтрока.Цена);
	...
	Если НЕ ТекущаяСтрока = Неопределено Тогда
		...
		Если НЕ (СуммаСтрокиТовара = 0) И(Скидка > 0.01 ИЛИ Скидка < -0.01) Тогда
			...
			СуммаСкидкиСтрокой = ОбщегоНазначенияРМККлиентСервер.ФорматСумм(Коэффициент * Скидка);
		...
		Если НЕ ОтображатьТолькоИтоги Тогда
			...
			ДанныеДляОтображения.ТоварСумма			= ОбщегоНазначенияРМККлиентСервер.ФорматСумм(ТекущаяСтрока.Сумма);
	...
	ДанныеДляОтображения.СуммаИтого		= ОбщегоНазначенияРМККлиентСервер.ФорматСумм(СуммаИтого);
	ДанныеДляОтображения.СуммаСкидки	= ОбщегоНазначенияРМККлиентСервер.ФорматСумм(СуммаСкидки);
