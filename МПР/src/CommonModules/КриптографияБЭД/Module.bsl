...
Функция ЗапросДействующихСертификатов(ИмяВременнойТаблицы, Отбор = Неопределено) Экспорт
	...
	Если Отбор.Облачные <> Неопределено Тогда
		Условие = ?(Отбор.Облачные, "Программа.ЭтоВстроенныйКриптопровайдер",
			"НЕ Программа.ЭтоВстроенныйКриптопровайдер");
...
Функция ЭтоСертификатыОблачногоСервиса(Сертификаты) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ПрограммыЭлектроннойПодписиИШифрования.ЭтоВстроенныйКриптопровайдер, ЛОЖЬ) КАК ЭтоВстроенныйКриптопровайдер,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЭтоВстроенныйКриптопровайдер Тогда
