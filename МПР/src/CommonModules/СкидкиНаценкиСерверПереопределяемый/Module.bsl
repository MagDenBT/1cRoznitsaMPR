...
Функция Рассчитать(Объект, ВходныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		//{ds-28.04.2021
		Если Объект.ИнтернетЗаказ 
			И ОбменССайтомРТ.ПоследнееСостояниеОплатыЗаказаПокупателя(Объект.Ссылка) = Перечисления.СостоянияОплатыЗаказаПокупателя.ОплаченПолностью 
			//{ds-26.07.2021
			Или Объект.ds_БезАвтоматическихСкидок Тогда
			//}	
				ВходныеПараметры.Вставить("ПрименятьКОбъекту", Ложь);		
		КонецЕсли;
		//}		
