...
Процедура УстановитьККТФФД12ПоДаннымОписанияТовара(ДанныеОписания, ПараметрыСканирования) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеОписания, "Номенклатура")
		И ЗначениеЗаполнено(ДанныеОписания.Номенклатура)
		И ЗначениеЗаполнено(ПараметрыСканирования.Склад) Тогда
		
		ПодключаемоеОборудование = РозничныеПродажиВызовСервера.КассаФискализацииРаспределяемойНоменклатуры(
			ПараметрыСканирования.Склад,
			МенеджерОборудованияВызовСервера.РабочееМестоКлиента(),
			ДанныеОписания.Номенклатура);
			
		Если ЗначениеЗаполнено(ПодключаемоеОборудование) Тогда
			ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ = Истина;
			ПараметрыСканирования.ККТФФД12ИСМП                   = ПодключаемоеОборудование;
		КонецЕсли;
