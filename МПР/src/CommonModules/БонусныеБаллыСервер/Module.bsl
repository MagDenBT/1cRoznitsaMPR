...
//
Функция ТаблицаМаксимальныхСуммОплаты(ТабличнаяЧастьТовары, БонуснаяПрограммаЛояльности, Магазин, ТабличнаяЧастьСкидкиНаценки = Неопределено) Экспорт //{ds +ТабличнаяЧастьСкидкиНаценки
	...
	//{ds
	Если Истина Тогда
		СписокТоваровИсключение = ds_ОбщегоНазначенияВызовСервера.СписокТоваровИсключениеПоКатегории(Магазин, ТабличнаяЧастьТовары);
		
		БонусыНеДействуютНаМаркетинговыеАкции = БонуснаяПрограммаЛояльности = Справочники.БонусныеПрограммыЛояльности.GMB;
		
		Если ТабличнаяЧастьСкидкиНаценки = Неопределено Тогда
			ТабличнаяЧастьСкидкиНаценки = Новый ТаблицаЗначений();
			ТабличнаяЧастьСкидкиНаценки.Колонки.Добавить("КлючСвязи");
			ТабличнаяЧастьСкидкиНаценки.Колонки.Добавить("СкидкаНаценка");	
		КонецЕсли;	
		
		ТабличнаяЧастьТоварыБезИсключений = ТабличнаяЧастьТовары.СкопироватьКолонки();
		Для Каждого Стр Из ТабличнаяЧастьТовары Цикл
			СтрСкидка = ТабличнаяЧастьСкидкиНаценки.Найти(Стр.КлючСвязи, "КлючСвязи");
			Если СписокТоваровИсключение.Найти(Стр.Номенклатура) = Неопределено 
				И ?(БонусыНеДействуютНаМаркетинговыеАкции, СтрСкидка = Неопределено Или Не ЗначениеЗаполнено(СтрСкидка.СкидкаНаценка), Истина) Тогда
				ЗаполнитьЗначенияСвойств(ТабличнаяЧастьТоварыБезИсключений.Добавить(), Стр);
			КонецЕсли;
		КонецЦикла;		
		
		Запрос.УстановитьПараметр("Товары", ТабличнаяЧастьТоварыБезИсключений);
	Иначе	
	//}
