...
Функция ПараметрыПечатьКопииЧека() Экспорт;
	...
	ПараметрыОперации = ПараметрыВыполненияОперации();        
	ПараметрыОперации.Вставить("ДокументОснование" , Неопределено); // Ссылка - Документ по которому пробивается чек.
	ПараметрыОперации.Вставить("НомерЧекаККТ" , Неопределено); // Строка - Номер фискального чека.
	ПараметрыОперации.Вставить("Аппаратно" , Ложь);  // Булево - Печать копии чека производиться аппаратно по данным ККТ, иначе по регистру фискальных операций.
