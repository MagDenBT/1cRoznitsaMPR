...
// Подготовка данных для пробития документа Чек ККМ.
//
// Параметры:
//  ЧекККМ - ДокументСсылка.ЧекиККМ - ссылка на документ для которого необходимо подготовить данные для пробития.
// 
// Возвращаемое значение:
//  ТаблицаТоваровЧека - ТаблицаЗначений - таблица в которой содержаться данные по товарам для пробития чека.
//
Функция ПодготовкаДанныхДляПробитияЧекаККМ(ЧекККМ) Экспорт
	
	ТаблицаТовары = ЧекККМ.Товары.Выгрузить();
	ТекстЗапроса = ОбщегоНазначенияРМКПереопределяемый.ТекстЗапросаДанныхДляПробитияЧека();
...
Функция НомерЧекаККМ(КассаККМ) Экспорт 
	...
	Возврат ОбщегоНазначенияРМКПереопределяемый.НомерЧекаККМ(КассаККМ);
...
//
Функция ПолучитьТипСистемыНалогообложенияККТ(Организация, ДатаДокумента = Неопределено) Экспорт
	
	Возврат ОбщегоНазначенияРМКПереопределяемый.ТипСистемыНалогообложенияККТ(Организация, ДатаДокумента);
...
Функция ЕстьПродукцияЕГАИС(ЧекККМ) Экспорт 
	Возврат ОбщегоНазначенияРМКПереопределяемый.ЕстьПродукцияЕГАИС(ЧекККМ);
КонецФункции
...
Функция РеквизитыКассыККМ(КассаККМ, РабочееМесто = Неопределено) Экспорт
	...
	Возврат ОбщегоНазначенияРМКПереопределяемый.РеквизитыКассыККМ(КассаККМ, РабочееМесто);
...
Функция АктуальнаяСтавкаНДСДляТовара(Объект, ТекущаяСтрока, СистемаНалогообложенияПоУмолчанию = Неопределено) Экспорт
	...
	Результат = ТекущаяСтрока.СтавкаНДС;
	
	Если Не ТекущаяСтрока = Неопределено И ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		Результат = ОбщегоНазначенияРМКПереопределяемый.АктуальнаяСтавкаНДСДляТовара(
			Объект,
			ТекущаяСтрока,
