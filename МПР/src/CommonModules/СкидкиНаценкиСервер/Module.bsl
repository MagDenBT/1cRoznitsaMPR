...
//
// Параметры:
//  Объект - ДокументОбъект - документ к которому необходимо применить результат расчета скидок.
...
Процедура РассчитатьСкидку(СтрокаДереваСкидок, Параметры)
	...
	//{ds-25.10.2020
	ТаблицаДляПроверки = СтрокаДереваСкидок.СтруктураДополнительныхДанных.ТаблицаТоваровПоСегментам.СкопироватьКолонки();
	Для Каждого Стр Из ТоварыСегмента Цикл ЗаполнитьЗначенияСвойств(ТаблицаДляПроверки.Добавить(), Стр) КонецЦикла;
	
	ds_ОбщегоНазначенияВызовСервера.ИсключитьИзРасчетаСкидокПоКатегорииНоменклатуры(Параметры.Магазин, СтрокаДереваСкидок.СкидкаНаценка, ТаблицаДляПроверки);
	
	Для Каждого Стр Из ТоварыСегмента Цикл 
		Если Не Стр.ИсключенаИзРасчета Тогда 
			СтрокиТаблицы = ТаблицаДляПроверки.НайтиСтроки(Новый Структура("КлючСвязи, ИсключенаИзРасчета", Стр.КлючСвязи, Истина));
			Если СтрокиТаблицы.Количество() Тогда 
				Стр.ИсключенаИзРасчета = Истина;	
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;	
	//}
	...
	ИначеЕсли СтрокаДереваСкидок.СпособПредоставления = Перечисления.СпособыПредоставленияСкидокНаценок.ВнешняяОбработка Тогда
		...
		Если СтрокаДереваСкидок.Публикация <> Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.Отключена Тогда
			...
			Попытка
				...
				//СтруктураВнешнихТаблиц = ds_ОбщегоНазначенияВызовСервера.Тест_РассчитатьСкидку(СтрокаДереваСкидок, ПараметрыВнешнейОбработки, Параметры);
				//}
...
Функция РассчитатьСуммуНДС(СтрокаТЧ, ЦенаВключаетНДС = Истина)
	
	Если СтрокаТЧ.СтавкаНДС = Перечисления.СтавкиНДС.НДС10 Тогда
		ПроцентНДС = 0.1;
	ИначеЕсли СтрокаТЧ.СтавкаНДС = Перечисления.СтавкиНДС.НДС18 Тогда
		ПроцентНДС = 0.18;
