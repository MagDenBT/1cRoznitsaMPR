...
Процедура Сформировать(СегментСсылка) Экспорт
	...
	ЭлементыОтбора = Настройки.Настройки.Отбор.Элементы;
	ПолеНоменклатура = Новый ПолеКомпоновкиДанных("Номенклатура");
	ЕстьПолеНоменклатура = Ложь;
	
	Для Каждого ЭлементОтбора Из ЭлементыОтбора Цикл 
		Если ЭлементОтбора.ЛевоеЗначение = ПолеНоменклатура И ЭлементОтбора.Использование Тогда 
			ЕстьПолеНоменклатура = Истина;
		КонецЕсли;	
	КонецЦикла;	
	
	Если Не ЕстьПолеНоменклатура Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Необходимо установить отбор по номенклатуре!");
		Возврат;
	КонецЕсли;	
	//}
