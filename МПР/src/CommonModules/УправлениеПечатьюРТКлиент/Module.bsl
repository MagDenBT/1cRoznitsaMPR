...
Процедура ОбработкаКомандыПечатиЦенниковИЭтикетокНаПринтереЭтикеток(ПараметрыПечати) Экспорт
	...
	Если ДанныеДляПринтераЭтикеток.Количество() > 0 Тогда
		...
		Для Каждого ТекМакет Из ДанныеДляПринтераЭтикеток Цикл
			...
			ПараметрыПечатиЭтикеток = ОборудованиеПринтерыЭтикетокКлиент.ПараметрыОперацииПечатиЭтикеток(ТекМакет.XML, ТекМакет.Этикетки);
			ОборудованиеПринтерыЭтикетокКлиент.НачатьПечатьЭтикеток(Неопределено, ИдентификаторФормы, Неопределено, ПараметрыПечатиЭтикеток);
...
Процедура ЗавершитьПечатьКопииЧекаДляФР(ПараметрДействия)
	...
	НомерЧека 					= ОбщегоНазначенияРТВызовСервера.ЗначениеРеквизитаОбъекта(ПараметрДействия.ОбъектыПечати[0], "НомерЧекаККМ");
		
	ПараметрДействия.Форма.Доступность = Ложь;
		
	ОповещенияПриЗавершении 	= Новый ОписаниеОповещения("ЗавершитьПовторнуюПечатьФискальногоЧека", ЭтотОбъект, ПараметрДействия);
	...
	ПараметрыОперации.Вставить("ДокументСсылка", ПараметрДействия.ОбъектыПечати[0]);
	
	ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьПечатьКопииЧека(
			ОповещенияПриЗавершении, Новый УникальныйИдентификатор, ИдентификаторУстройстваФР, ПараметрыОперации);
...
Процедура ЗавершитьПечатьТоварногоЧекаДляФР(ПараметрДействия)
	...
	ПараметрыОперации = Новый Структура("СтрокиТекста", СтрокаПечати);
	ОповещенияПриЗавершение = Новый ОписаниеОповещения("ЗавершитьПечатьТоварногоЧекаДляФРЗавершение", ЭтотОбъект, ПараметрДействия);
	ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьПечатьТекста(ОповещенияПриЗавершение, Новый УникальныйИдентификатор, ИдентификаторУстройстваФР, ПараметрыОперации);
