...
//
Функция EF_00_00452396_ВыполнитьКонтрольСопоставленияНоменклатуры(Знач ДанныеЭлектронногоДокумента = Неопределено,
...
Функция ВыполнитьКонтрольСопоставленияНоменклатуры(Знач ДанныеЭлектронногоДокумента = Неопределено,
	...
	Для Каждого НоменклатураКонтрагента Из НаСопоставление Цикл
		...
		Для Каждого Элемент Из Сопоставление Цикл
			...
			ИсключаемыеСвойства = EF_00_00452396_ИсключаемыеИзКонтроляСопоставленияСвойстваНоменклатуры(ДанныеДокумента);
			Для Каждого ЭлементСвойства Из ИсключаемыеСвойства Цикл
...
Функция EF_00_00452396_ИсключаемыеИзКонтроляСопоставленияСвойстваНоменклатуры(ДанныеДокумента)
...
Процедура ИсключитьНеОтражаемыеВУчетеЭлектронныеДокументы(Параметры, Результат)
	...
	Для каждого СтрокаПараметров Из Параметры Цикл
		...
		Запрос.УстановитьПараметр("ИдентификаторСвязи", СтрокаПараметров.ЭлектронныйДокумент.ИдентификаторСвязи);
		РезультатЗапроса = Запрос.Выполнить();
			...
			Если ТипЗнч(СтрокаПараметров.ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО") Тогда
				...
				Попытка
					...
					Если РТ_ЭтоПеремещениеТоваров Тогда
						...
						МенеджерЗаписи.ЭлектронныйДокумент = СтрокаПараметров.ЭлектронныйДокумент;
					...
					Если РТ_ЭтоСчетФактураВыданная Тогда
						...
						Если СтрокаПараметров.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД
