...
Процедура ЗаполнитьРегистрационныеДанныеОрганизации(Организация, ДанныеОрганизации) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		СведенияОбАдресе = РаботаСАдресами.СведенияОбАдресе(Выборка.ЗначенияПолей);
		ДанныеОрганизации.Индекс = СведенияОбАдресе.Индекс;
		ДанныеОрганизации.Регион = Строка(АдресныйКлассификатор.КодРегионаПоНаименованию(СокрЛП(СведенияОбАдресе.Регион)));
