...
//
Функция ТорговыеТочкиСверкиВзаиморасчетов() Экспорт
	...
	Запрос.Текст =
		...
		|	И НастройкиИнтеграцииСПлатежнымиСистемами.СверкаВзаиморасчетов";
...
// Определяет доступность проведения сверки взаиморасчетов для платежной
// системы.
//
// Параметры:
//  ПлатежнаяСистема - ПеречислениеСсылка.ПлатежныеСистемы - платежная систем для проверки.
//
// Возвращаемое значение:
//  Булево - если Истина, сверка взаиморасчетов доступна.
//
Функция ПоддерживаетсяСверкаВзаиморасчетов(ПлатежнаяСистема) Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СверкаВзаиморасчетовПлатежныеСистемы") Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Возврат ПлатежныеСистемы.Найти(ПлатежнаяСистема) <> Неопределено;
...
//      следует использовать метод ИнтеграцияСПлатежнымиСистемами.ПлатежныеСистемыДляВозврата;
		...
		Если ЗаданияОбновления.Количество() = 0 Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					...
					Метаданные.РегламентныеЗадания.ОбновлениеНастроекУчастниковСБП,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Выполнить обновление настроек СБП.
							|%1'"),
