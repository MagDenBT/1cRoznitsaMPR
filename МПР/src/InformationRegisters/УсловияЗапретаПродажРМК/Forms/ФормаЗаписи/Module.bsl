...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Запись.ОсобенностьУчета) Тогда
		...
		ПереченьВидовНоменклатуры = ВидыНоменклатурыПоОсобенностиУчета(Запись.ОсобенностьУчета);
		Элементы.ВидНоменклатуры.СписокВыбора.ЗагрузитьЗначения(ПереченьВидовНоменклатуры);
...
Функция ОрганизацияПоДаннымКассыТекущегоРабочегоМеста()
	...
	Результат = ОбщегоНазначенияРМКПереопределяемый.ПустаяСсылкаНаСправочник("Организации");
	
	РабочееМесто = ОбщегоНазначенияРМКПереопределяемый.ТекущееРабочееМесто();
	ТорговыйОбъект = ОбщегоНазначенияРМКПереопределяемый.ТекущийМагазин();
