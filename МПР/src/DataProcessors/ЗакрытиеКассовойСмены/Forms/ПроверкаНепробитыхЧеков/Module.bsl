...
Процедура ОтчетБезГашения(Команда)
	...
	ОборудованиеЧекопечатающиеУстройстваКлиент.НачатьФормированиеОтчетаБезГашения(Оповещение, УникальныйИдентификатор, ИдентификаторУстройстваФР, ПараметрыОперации);
	...
	Если Результат Тогда
		...
		Если ПараметрыВыполнения.ТекущийЭлемент < ПараметрыВыполнения.КоличествоЭлементов Тогда
			...
			Если ЭлементОсновнойТаблицы.Пометка Тогда
				...
				ИначеЕсли ЭлементОсновнойТаблицы.ВидДокумента = "РегистрацияБезналичнойОплаты" Тогда
					ПараметрыЧека = ПараметрыЧека(ДокументНаККМ);
					...
					Иначе
						...
						Оповещение = Новый ОписаниеОповещения("НапечататьЧекКлиентЗавершениеРБО", ЭтотОбъект, Контекст);
...
Процедура ПробитьЧекВыполнитьЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если РезультатПроведения = Истина Тогда
			НомерДокументаКассыККМ[Объект.КассаККМ] = РезультатВыполнения.НомерЧекаККТ + 1;
...
Процедура ПробитьЧекВыполнить(ОповещениеПриЗавершении, ЧекККМ)
	...
	Если ИдентификаторУстройстваФР <> Неопределено Тогда
		...
		НомерЧека = НомерДокументаКассыККМ[Объект.КассаККМ] + 1;
		ОбщиеПараметры = ПодготовитьДанныеДляПробитияЧека(ЧекККМ, НомерЧека);
...
Процедура НапечататьЧекКлиентЗавершениеРКО(РезультатВыполнения, ПараметрыОперации) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		НапечататьЧекСерверЗавершение(РезультатВыполнения.НомерЧекаККТ, ПараметрыОперации.ДокументНаККМ, Отказ);
...
Процедура НапечататьЧекКлиентЗавершениеПКО(РезультатВыполнения, ПараметрыОперации) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		ПараметрыОперации.Вставить("НомерЧекаККМ", РезультатВыполнения.НомерЧекаККТ);
		...
		ПараметрыИнкассацииОперации = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыИнкассации();
