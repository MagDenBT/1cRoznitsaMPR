...
&НаСервереБезКонтекста
Функция EF_00_00451459_ПоляРаздела(Знач Раздел, Знач ПакетноеОтображение)
...
Функция ПоляРаздела(Знач Раздел, Знач ПакетноеОтображение)
	...
	Если НЕ ОбщегоНазначения.ЭтоМобильныйКлиент() И  ПакетноеОтображение Тогда
		...
		Если РазделыБезДатыПакета.Найти(Раздел) = Неопределено Тогда
			...
			ПоляРаздела = EF_00_00451459_ЗаменитьПоляРаздела(ПоляРаздела, ПараметрыЗамены);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция EF_00_00451459_ЗаменитьПоляРаздела(Знач СтрокаПолей, Знач ПараметрыЗамены)
