...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = РаботаСНоменклатуройКлиент.ОписаниеОповещенийПодсистемы().ЗагрузкаНоменклатуры Тогда
		Если Параметр.СозданныеОбъекты.Количество() > 0 Тогда
			...
			СтруктураПараметров.Вставить("Номенклатура", Параметр.СозданныеОбъекты[0].Номенклатура);
			СтруктураПараметров.Вставить("Родитель"    , ОбщегоНазначенияРТВызовСервера.ЗначениеРеквизитаОбъекта(Параметр.СозданныеОбъекты[0].Номенклатура,"Родитель"));
			ЗавершитьОбработкуДанныхПоКодуКлиент(Новый Структура("АктивизироватьСтроку",СтруктураПараметров));
		КонецЕсли;
	КонецЕсли;
	// Конец ЭлектронноеВзаимодействие.РаботаСНоменклатурой
КонецПроцедуры
...
Процедура ds_ПолучитьНоменклатуруИзБуфера(Команда)
	...
	Для Каждого СтруктураПараметры Из МассивНоменклатуры Цикл	
		НоваяСтрока = Новый Структура;
		НоваяСтрока.Вставить("Номенклатура",               СтруктураПараметры.Номенклатура);
		НоваяСтрока.Вставить("Характеристика",             ПредопределенноеЗначение("Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка"));
		НоваяСтрока.Вставить("ХарактеристикиИспользуются", Ложь);
		НоваяСтрока.Вставить("ТипНоменклатуры",            СтруктураПараметры.ТипНоменклатуры);
		НоваяСтрока.Вставить("Упаковка",                   ПредопределенноеЗначение("Справочник.УпаковкиНоменклатуры.ПустаяСсылка"));
		НоваяСтрока.Вставить("Цена",                       0);
		НоваяСтрока.Вставить("ДоговорКонтрагента",    	   ПредопределенноеЗначение("Справочник.ДоговорыКонтрагентов.ПустаяСсылка"));
		НоваяСтрока.Вставить("КоличествоУпаковок",         1);
		
		ДобавитьВКорзину(НоваяСтрока, 1);
	КонецЦикла;		
...
Процедура ОбработатьФайлНаСервереЗавершение(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	...
	Если ОписаниеПомещенногоФайла <> Неопределено Тогда
		МассивНоменклатуры = ПрочитатьФайлНаСервере(ОписаниеПомещенногоФайла.Адрес, ОписаниеПомещенногоФайла.СсылкаНаФайл.Имя, ДополнительныеПараметры);
...
&НаСервере
Функция ПрочитатьФайлНаСервере(Адрес, ИмяФайла, ДополнительныеПараметры)
	...
	Для Каждого Стр Из Результат Цикл 
		...
		НоваяСтрока.Вставить("ТипНоменклатуры", Стр.ТипНоменклатуры);
		
		МассивНоменклатуры.Добавить(НоваяСтрока);
	КонецЦикла;	
