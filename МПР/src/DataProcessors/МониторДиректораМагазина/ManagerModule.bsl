...
Функция РассчитатьПоказатель_ДанныеПоЧекам(ПараметрыРасчета)
	...
	//{ds-13.07.2021-средний чек считается некорректно, меняем
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЕСТЬNULL(СРЕДНЕЕ(ЧекККМ.СуммаДокумента), 0) КАК Значение
	|ИЗ
	...
	Запрос.УстановитьПараметр("Магазины", ПараметрыЗадания.СписокМагазинов);	
	Запрос.УстановитьПараметр("НачалоПериода", НачалоДня(ПараметрыЗадания.Период.ДатаНачала));
	Запрос.УстановитьПараметр("КонецПериода", КонецДня(ПараметрыЗадания.Период.ДатаОкончания));
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		СреднийЧек = Окр(Выборка.Значение, 0);	
	КонецЕсли;	
	//}
