...
Процедура ВыгрузитьВыполнить(Команда)
	...
	ПараметрыОперации.ТаблицаТоваров = МассивБазыТоваров();
	ПараметрыОперации.ЧастичнаяВыгрузка = НЕ ВариантЗаполнения = "ПоДокументу";
	
	ОповещенияПриЗавершение = Новый ОписаниеОповещения("ВыгрузитьВыполнитьЗавершение", ЭтотОбъект);
	ОборудованиеТерминалыСбораДанныхКлиент.НачатьВыгрузкуДанныеВТСД(ОповещенияПриЗавершение, УникальныйИдентификатор,, ПараметрыОперации);
...
Процедура ОчиститьДанныеВТСД(Команда)
	...
	ОборудованиеТерминалыСбораДанныхКлиент.НачатьОчисткуДанныеВТСД(ОповещенияПриЗавершение, УникальныйИдентификатор);
...
Процедура ЗаполнитьПоТоварамДокументаНаСервере()
	...
	ИначеЕсли ДокументИмяТипа = "УстановкаСебестоимости" Тогда
		...
		|	Штрихкоды.Штрихкод";
	//{ds-28.01.2021	
	ИначеЕсли ДокументИмяТипа = "ds_АктПриемкиТоваров" Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ds_АктПриемкиТоваровТовары.Номенклатура КАК Номенклатура,
		|	Штрихкоды.Упаковка КАК Упаковка,
		|	ds_АктПриемкиТоваровТовары.Характеристика КАК Характеристика,
		|	ds_АктПриемкиТоваровТовары.КоличествоПлан КАК КоличествоУпаковок,
		|	ЦеныНоменклатурыСрезПоследних.Цена КАК Цена,
		|	Штрихкоды.Штрихкод КАК Штрихкод
		|ИЗ
		|	Документ.ds_АктПриемкиТоваров.Товары КАК ds_АктПриемкиТоваровТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
		|		ПО ds_АктПриемкиТоваровТовары.Номенклатура = Штрихкоды.Владелец
		|			И ds_АктПриемкиТоваровТовары.Характеристика = Штрихкоды.Характеристика
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних КАК ЦеныНоменклатурыСрезПоследних
		|		ПО ds_АктПриемкиТоваровТовары.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
		|			И ds_АктПриемкиТоваровТовары.Характеристика = ЦеныНоменклатурыСрезПоследних.Характеристика
		|			И ds_АктПриемкиТоваровТовары.Ссылка.Магазин.ПравилоЦенообразования.ВидЦен = ЦеныНоменклатурыСрезПоследних.ВидЦены
		|ГДЕ
		|	ds_АктПриемкиТоваровТовары.Ссылка = &Документ
		|
		|УПОРЯДОЧИТЬ ПО
		|	ds_АктПриемкиТоваровТовары.НомерСтроки,
		|	Штрихкоды.Штрихкод";
	//}		
	Иначе
		Запрос = Новый Запрос;
...
Функция МассивБазыТоваров()
	...
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ * ПОМЕСТИТЬ Товары ИЗ &Товары КАК Товары; ВЫБРАТЬ *, Номенклатура.Код КАК Артикул ИЗ Товары КАК Товары";
	Запрос.УстановитьПараметр("Товары", Объект.Товары.Выгрузить());
