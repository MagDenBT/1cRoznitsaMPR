...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ТорговыеПредложения") Тогда
		...
		СтрокаРегионПрофиля = Элементы.РегионРежимОтбора.СписокВыбора.НайтиПоЗначению(1);
		
		Если ЗначениеЗаполнено(СтрокаРегионПрофиля) Тогда
			Элементы.РегионРежимОтбора.СписокВыбора.Удалить(СтрокаРегионПрофиля);
		КонецЕсли;
