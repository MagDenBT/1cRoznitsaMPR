...
Процедура ПриОткрытии(Отказ)
	...
	Если (ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.ПоступлениеТоваров")
				Или	((ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.ПеремещениеТоваров")) И (ПараметрыРаспределения.Операция = "Приемка"))
			И Не ds_ОбщегоНазначенияВызовСервера.РольДоступнаТекущемуПользователю("ПолныеПрава")) Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Приемка выполняется документом Акт приемки.",,,, Отказ);
	КонецЕсли;	
	//}
