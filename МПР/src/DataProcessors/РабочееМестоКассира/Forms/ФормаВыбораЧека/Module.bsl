...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТаблицаЧековККМ.ТекстЗапроса = ОбщегоНазначенияРМКПереопределяемый.ТекстЗапросаТаблицаЧековККМ();
	...
	ИначеЕсли Параметры.Свойство("ВыборДокументаРассрочки") Тогда
		...
		ЗаполнитьТаблицуЧековККМОплаченыхВРассрочку();
...
Процедура ВыполнитьПоискЧекаККМ(Штрихкод)
	...
	Если РасшифровкаКода.Расшифрован Тогда
		...
		Запрос = Новый Запрос();
		Запрос.Текст = ОбщегоНазначенияРМКПереопределяемый.ТекстЗапросаЧекПоФискальномуПризнаку();
	...
	Иначе
		...
		|	И ТИПЗНАЧЕНИЯ(ФискальныеОперации.ДокументОснование) = &ТипЧекККМ");
		Запрос.УстановитьПараметр("ФискальныйПризнак", Штрихкод);
		Запрос.УстановитьПараметр("ТипЧекККМ", ТипЗнч(ОбщегоНазначенияРМКПереопределяемый.ПустаяСсылкаНаЧекККМ()));
	...
	Если НайденныеЧеки.Количество() > 0 Тогда
		...
		СтатусыПробитогоЧека = Новый Массив;
		ОбщегоНазначенияРМКПереопределяемый.ЗаполнитьСтатусыПробитыхЧековККМ(СтатусыПробитогоЧека);
		
		ВидыОперацииПродажа = ОбщегоНазначенияРМКПереопределяемый.ВидОперацииЧекаККМ();
...
&НаСервере
Процедура ЗаполнитьТаблицуЧековККМОплаченыхВРассрочку()
	ОбщегоНазначенияРМКПереопределяемый.ЗаполнитьТаблицуЧековККМОплаченыхВРассрочку(ЭтотОбъект);
КонецПроцедуры
