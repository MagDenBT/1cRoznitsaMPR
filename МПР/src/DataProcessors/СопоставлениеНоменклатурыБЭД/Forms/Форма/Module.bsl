...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//{ds
	ds_ЭлектронныеДокументы.ЗагрузитьЗначения(Параметры.ЭлектронныеДокументы);
	//}
...
&НаКлиенте
Процедура ds_ПодобратьПоКоду(Команда)
	
	ds_ПодобратьПоКодуНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ds_ПодобратьПоКодуНаСервере()
	
	ТабТовары = Новый ТаблицаЗначений;
	ТабТовары.Колонки.Добавить("Код", ОбщегоНазначения.ОписаниеТипаСтрока("11"));
	ТабТовары.Колонки.Добавить("Наименование", ОбщегоНазначения.ОписаниеТипаСтрока("200"));
	
	ДанныеДокументов = ЭлектронныеДокументыЭДО.ДанныеДокументовДляОтраженияВУчете(ds_ЭлектронныеДокументы);
	
	Если Не ДанныеДокументов.Количество() Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыПолучения = ЭлектронныеДокументыЭДО.НовыеПараметрыПолученияДанныхДокумента();
	ПараметрыПолучения.ОсновнойФайл = ДанныеДокументов[0].ДанныеОсновногоФайла;
	...
	Для Каждого Стр Из ТаблицаТоваров Цикл
		Если ЗначениеЗаполнено(Стр.Сопоставление.Артикул)
			И ЗначениеЗаполнено(Стр.Сопоставление.Наименование) Тогда 
			НовСтр = ТабТовары.Добавить();
			НовСтр.Код = Стр.Сопоставление.Артикул;
			НовСтр.Наименование = Стр.Сопоставление.Наименование;
		КонецЕсли;	
	КонецЦикла;	
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТабТовары.Код КАК Код,
	|	ТабТовары.Наименование КАК Наименование
	|ПОМЕСТИТЬ ТабТовары
	|ИЗ
	...
	|		ПО ТабТовары.Код = Номенклатура.Код";
	Запрос.УстановитьПараметр("ТабТовары", ТабТовары);
	ТЗ = Запрос.Выполнить().Выгрузить();
	
	Для Каждого Стр Из ТЗ Цикл
		ИскСтроки = Объект.Сопоставление.НайтиСтроки(Новый Структура("Наименование", Стр.Наименование));
		Если ИскСтроки.Количество() И ИскСтроки[0].Номенклатура.Пустая() Тогда 
			ИскСтроки[0].Номенклатура = Стр.Номенклатура;
			ИскСтроки[0].Артикул = Стр.Код;		
			ИскСтроки[0].ПоискВариантовНеТребуется = Истина;
			ИскСтроки[0].ПодсказкаНайденныхВариантовХарактеристики = "Характеристика";
			ИскСтроки[0].Сопоставлено = Истина;	
		КонецЕсли;	
	КонецЦикла;
