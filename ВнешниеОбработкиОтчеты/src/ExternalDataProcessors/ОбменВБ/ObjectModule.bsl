...
Процедура ЗаполнитьТаблицуЗаказов() Экспорт
	...
	Если СтруктураЗаказы.УспехЗапроса Тогда
		...
		Для Каждого Заказ Из СтруктураЗаказы.Результат Цикл
			...
			стрТЧ.СкладWB = Заказ.subject;
			стрТЧ.СтикерТовараWB = Заказ.subject;
			стрТЧ.АртикулWB = Заказ.subject;
			стрТЧ.ШтрихкодWB = Заказ.subject;
			Если ПоследнийШтрихкод = Заказ.barcode Тогда
				стрТЧ.Номенклатура = ПоследняяНоменклатура;
			Иначе
				Запрос = ЗапросПоНоменклатуреКонтрагентов();
				Запрос.УстановитьПараметр("Владелец", КонтрагентДляПривязкиТоваров);
				Запрос.УстановитьПараметр("ШтрихкодКомбинации", Заказ.barcode);
...
Процедура СоздатьЗаказыПоТЧ() Экспорт
	...
	КонецЕсли;
		...
		СгруппированныеЗаказы = ЗаказыWB.Выгрузить("gNumber");
		СгруппированныеЗаказы.Свернуть("gNumber");
		...
		Для Каждого ЗаказID Из СгруппированныеЗаказы Цикл
			...
			Если ЗначениеЗаполнено(НайтиЗаказВ1с(ЗаказID.IdЗаказаWB)) Тогда //Спорное решение. Если обработка будет несколькими пользователям, то нельзя допустить задублирования заказов
				Продолжить;
			КонецЕсли;
				СоздатьЗаказ(ЗаказID.IdЗаказаWB);
		КонецЦикла;
...
&НаСервере
Процедура СоздатьЗаказ(IDЗаказа)  
	
	ПроводитьДокументы = True;     //ToDo
	
		ТоварыПоЗаказу = ЗаказыWB.НайтиСтроки(новый Структура("IDЗаказа",IDЗаказа));
...
&НаСервере
Процедура ЗаполнитьРеквизитыЗаказа(Док,строкаТЧ)
	
	
	
	Док.НомерЗаказаНаСайте	= строкаТЧ.IdЗаказаWB; //ToDo	
	Док.ДатаЗаказаНаСайте	= строкаТЧ.ДатаЗаказаWB; //ToDo	
	...
	Док.АдресДоставки = строкаТЧ.СкладWB; //ToDo
	
	ЗаполнитьСтрокуЗаказа(Док,строкаТЧ); 
...
&НаСервере
Процедура ЗаполнитьСтрокуЗаказа(Док, СтрокиТЧ)   
	
	Для Каждого стрДанных Из СтрокиТЧ Цикл
		
	Если Не ЗначениеЗаполнено(стрДанных.Номенклатура) Тогда
		ВызватьИсключение ("По товару " + стрДанных.ТоварWB + " в строке " + стрДанных.НомерСтроки + " нет привязанной номенклатуры в 1с.Заказ создать невозможно. Сначала сделайте привязку");
	КонецЕсли;
	...
	стрДанных.СуммаТовараWB, 
	стрДанных.СуммаТовараWB / (1 + ПолучитьСтавкуНДС(СтрДок.СтавкаНДС) / 100));   	
	...
	КонецЕсли;
	СтрДок.СтикерWB = стрДанных.СтикерТовараWB;
...
Функция ПолучитьЗаказыНаWBv1()
	...
	ДатаНачало = ДатаRFC3339Кодированная(НачальнаяДата);
	АдресРесурса = "/supplier/orders?key=" + ТокенAPIv1 + "&dateFrom=" + ДатаНачало;
	ОтветСтруктурой = ЗапросНаСерверWB("GET",АдресСервисаAPIv1, АдресРесурса, Заголовки);
