...
//			КонецЕсли;
			...
			стрТЧ.СуммаТовараWB = Заказ.totalPrice / 100;
...
Функция СоздатьЗаказ(IdСкладаWB)
	...
	Если ГлавныйОтвет.УспехЗапроса Тогда
		Для Индекс = 1 По ВсеПорции.ВГраница() Цикл
			...
			Если Ответ.УспехЗапроса Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ГлавныйОтвет.Результат.data,
					Ответ.Результат.data, Истина);
			Иначе
...
Функция ВызватьМетодЗапроса(Запрос, Соединение, Метод)
	...
	Исключение
		...
		Ошибка = Истина;
	КонецПопытки;
	ОтветСтруктурой = Новый Структура("Результат,УспехЗапроса,КодСостояния", Результат, Не Ошибка, РезультатЗапрос.КодСостояния);
