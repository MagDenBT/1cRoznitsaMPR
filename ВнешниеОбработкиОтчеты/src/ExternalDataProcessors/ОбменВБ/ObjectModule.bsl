...
Процедура ЗаполнитьСопоставлениеНоменклатуры() Экспорт
	...
	Если СтруктураТовары.УспехЗапроса Тогда
		...
		ТЗ.Колонки.Добавить("АртикулWB", ОбщегоНазначения.ОписаниеТипаСтрока(255)); 
		ТЗ.Колонки.Добавить("НормализованныйАртикулWB", ОбщегоНазначения.ОписаниеТипаСтрока(255));
		ТЗ.Колонки.Добавить("НаименованиеWB", ОбщегоНазначения.ОписаниеТипаСтрока(255));
		ТЗ.Колонки.Добавить("ШтрихкодWB", ОбщегоНазначения.ОписаниеТипаСтрока(255));
		ТЗ.Колонки.Добавить("БрендWB", ОбщегоНазначения.ОписаниеТипаСтрока(255));
		ТЗ.Колонки.Добавить("КатегорияWB", ОбщегоНазначения.ОписаниеТипаСтрока(255));
		ТЗ.Колонки.Добавить("IDНоменклатурыWB", ОбщегоНазначения.ОписаниеТипаСтрока(255));
		
		Для Каждого Стр Из СтруктураТовары.Результат.stocks Цикл 
			НовСтр = ТЗ.Добавить();
			НовСтр.АртикулWB = Стр.article;
			НовСтр.НаименованиеWB = Стр.name;
			НовСтр.ШтрихкодWB = Стр.barcode;
			НовСтр.БрендWB = Стр.brand;
			НовСтр.КатегорияWB = Стр.subject;
			НовСтр.IDНоменклатурыWB = Формат(Стр.nmId, "ЧГ="); 
			НовСтр.НормализованныйАртикулWB = НовСтр.АртикулWB;
			Если СтрДлина(НовСтр.НормализованныйАртикулWB) < 9 Тогда 
				ОставитьТолькоЦифрыВСтроке(НовСтр.НормализованныйАртикулWB);
				НовСтр.НормализованныйАртикулWB = Сред(НовСтр.НормализованныйАртикулWB, 1, 3) + "-" + Сред(НовСтр.НормализованныйАртикулWB, 4, 3);
			КонецЕсли;	
		КонецЦикла;	
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ТЗ.АртикулWB КАК АртикулWB,
		|	ТЗ.НормализованныйАртикулWB КАК НормализованныйАртикулWB,
		|	ТЗ.НаименованиеWB КАК НаименованиеWB,
		|	ТЗ.ШтрихкодWB КАК ШтрихкодWB,
		|	ТЗ.БрендWB КАК БрендWB,
		|	ТЗ.КатегорияWB КАК КатегорияWB,
		|	ТЗ.IDНоменклатурыWB КАК IDНоменклатурыWB
		|ПОМЕСТИТЬ ТЗ
		|ИЗ
		|	&ТЗ КАК ТЗ
		|;
		...
		|	ТЗ.КатегорияWB КАК КатегорияWB,
		|	ТЗ.IDНоменклатурыWB КАК IDНоменклатурыWB,
		|	НоменклатураКонтрагентовБЭД.Номенклатура КАК НоменклатураСопоставлено,
		|	ЕСТЬNULL(НоменклатураКонтрагентовБЭД.Номенклатура, спрНоменклатура.Ссылка) КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА НоменклатураКонтрагентовБЭД.Номенклатура.Код ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		...
		|		ПО ТЗ.IDНоменклатурыWB = НоменклатураКонтрагентовБЭД.Идентификатор
		|			И (НоменклатураКонтрагентовБЭД.Владелец = &Поставщик)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК спрНоменклатура
		|		ПО ТЗ.НормализованныйАртикулWB = спрНоменклатура.Артикул
		|
		|УПОРЯДОЧИТЬ ПО
		|	IDНоменклатурыWB";
		Запрос.УстановитьПараметр("Поставщик", КонтрагентДляПривязкиТоваров);
		Запрос.УстановитьПараметр("ТЗ", ТЗ);
		
		СопоставлениеНоменклатуры.Загрузить(Запрос.Выполнить().Выгрузить());
...
//			КонецЕсли;
			...
			Если ДатаЗаказа + ВремяОтлежкиЗаказа * 60 > ТекущаяДата Тогда
				Продолжить;
			КонецЕсли;
			стрТЧ = ЗаказыWB.Добавить();
			стрТЧ.ДатаЗаказаWB = ДатаЗаказа;
...
Процедура ДобавитьЗаказыКТЧ()

	Запрос = ЗапросЗаказыВ1с();
	Запрос.УстановитьПараметр("МассивИдентификаторов", ЗаказыWB.ВыгрузитьКолонку("IdЗаказаWB"));
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		стрТЧ = ЗаказыWB.Найти(Выборка.ds_ИдентификаторЗаказа, "IdЗаказаWB");
		стрТЧ.Заказ1с = Выборка.Заказ;
		стрТЧ.Заказ1сСоздан = Истина;
	КонецЦикла;

//	Запрос = ЗапросЗаказыВ1с();
//	Для Каждого стрТЧ Из ЗаказыWB Цикл
//		Запрос.УстановитьПараметр("Идентификатор", стрТЧ.IdЗаказаWB);
//		Выборка = Запрос.Выполнить().Выбрать();
//			Пока Выборка.Следующий() Цикл
//			стрТЧ.Заказ1с = Выборка.Заказ;
//			стрТЧ.Заказ1сСоздан = Истина;
	...
	Если СтруктураПоставки.УспехЗапроса Тогда
		...
		Для Каждого стр Из СтруктураПоставки.Результаты Цикл
			...
			Для Каждого поставка Из стр.Результат.supplies Цикл
				...
				Штрихкод = ПолучитьШтрихкодПоставкиWBv2(поставка.supplyId);
				новаяСтрока.ШтрихкодПоставкиWBBase64 = Штрихкод.file;
				новаяСтрока.ШтрихкодПоставкиWB = Штрихкод.name;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура СвязатьТовары() Экспорт
	...
	Для Каждого стр Из СопоставлениеНоменклатуры Цикл
		...
		Если Стр.Сопоставлено Или Не ЗначениеЗаполнено(Стр.Номенклатура) Тогда
			Продолжить;
		КонецЕсли;
...
Функция СоздатьПоставкуВ1с(стрПоставка, МассивЗаказовСТоварами)
	...
	ДобавитьСтикерыКЗаказам(Док.Товары.Выгрузить( , "ЗаказПокупателя,КодСтроки"));
...
Процедура ЗаполнитьРеквизитыНакладной(Док, стрПоставка, МассивЗаказовСТоварами)
	...
	Док.ds_РеализацияПоЗаказам = Истина;
	Док.ds_СтатусПоставки = СтатусыПоставок().Найти(стрПоставка.СтатусWB).Статус1с;
	Док.ds_ШтрихкодПоставкиWB = стрПоставка.ШтрихкодПоставкиWB;
	...
	ОшибкиТоварнойЧасти = "";
	ЗаполнитьСтрокиНакладной( Док, МассивЗаказовСТоварами);
	ОбработатьДокументОснованиеРеализации(Док);
...
Процедура ОбработатьДокументОснованиеРеализации(Док)
	...
	|ПОМЕСТИТЬ ВТ
	|ИЗ
	|	&ТабличнаяЧасть КАК ВременнаяТаблица
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МИНИМУМ(ВТ.ЗаказПокупателя) КАК ЗаказПокупателя,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ.ЗаказПокупателя) КАК КоличествоЗаказов
	|ИЗ
	|	ВТ КАК ВТ";

	Запрос.УстановитьПараметр("ТабличнаяЧасть", Док.Товары.Выгрузить());
	Выборка = Запрос.Выполнить().Выбрать();

	Выборка.Следующий();

	Если Выборка.КоличествоЗаказов = 1 Тогда
		Док.ЗаказПокупателя = Выборка.ЗаказПокупателя;
	КонецЕсли;

КонецПроцедуры

Процедура ЗаполнитьСтрокиНакладной(Док, МассивЗаказовСТоварами)
	НесохраненныеЗаказыWB = Новый Массив;
	ПоследнийШтрихкод = "";
	ПоследняяНоменклатура = Неопределено;
	ЗапросПривязНомен = ЗапросПоНоменклатуреКонтрагентов();
	ЗапросПривязНомен.УстановитьПараметр("Владелец", КонтрагентДляПривязкиТоваров);
	ЗапросЗаказы1с = ЗапросЗаказыВ1с();
	Для Каждого стрТовар Из МассивЗаказовСТоварами Цикл
		...
		ЗапросЗаказы1с.УстановитьПараметр("Идентификатор", стрТовар.orderId);
		ВыборкаЗаказ = ЗапросЗаказы1с.Выполнить().Выбрать();

		Если Не ВыборкаЗаказ.Следующий() Тогда
		...
		СтрДок.ЗаказПокупателя = ВыборкаЗаказ.Заказ;
		СтрДок.КодСтроки = ВыборкаЗаказ.КодСтроки;
	...
	Если НесохраненныеЗаказыWB.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		ЗаполнитьСтрокиНакладной(Док, НесохраненныеЗаказыWB);
	КонецЕсли;
...
Функция ЗапросЗаказыВ1с()
	...
	|	ВЫБОР
	|		КОГДА НЕ &Идентификатор = Неопределено
	|			ТОГДА ЗаказыПокупателей.Заказ.Товары.ds_ИдентификаторЗаказа = &Идентификатор
	|		ИНАЧЕ ЗаказыПокупателей.Заказ.Товары.ds_ИдентификаторЗаказа В (&МассивИдентификаторов)
	|	КОНЕЦ
	|	И ЗаказыПокупателей.Заказ.Проведен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗаказПокупателяТовары.ds_ИдентификаторЗаказа КАК ds_ИдентификаторЗаказа,
	|	ЗаказПокупателяТовары.Ссылка КАК Заказ,
	|	ЗаказПокупателяТовары.КодСтроки
	|ИЗ
	|	ВТ КАК ВТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
	|		ПО ВТ.Заказ = ЗаказПокупателяТовары.Ссылка
	|ГДЕ
	|	ВЫБОР
	|		КОГДА НЕ &Идентификатор = Неопределено
	|			ТОГДА ЗаказПокупателяТовары.ds_ИдентификаторЗаказа = &Идентификатор
	|		ИНАЧЕ ЗаказПокупателяТовары.ds_ИдентификаторЗаказа В (&МассивИдентификаторов)
	|	КОНЕЦ";

	Запрос.УстановитьПараметр("МассивИдентификаторов", Неопределено);
	Запрос.УстановитьПараметр("Идентификатор", Неопределено);
...
Процедура ДобавитьСтикерыКЗаказам(тзЗаказПокупателяИКодСтроки)

	Данные = СформироватьТаблицуСтикеров(тзЗаказПокупателяИКодСтроки);
	Если Не ЗначениеЗаполнено(Данные) Тогда
		Возврат;
	КонецЕсли;
	тзЗаказы = Данные.Скопировать( , "ЗаказПокупателя");
	тзЗаказы.Свернуть("ЗаказПокупателя");
	Для Каждого заказ Из тзЗаказы Цикл
		НачатьТранзакцию();
		Попытка
			ДокОбъект =	Заказ.ЗаказПокупателя.ПолучитьОбъект();
			тзСтикеры = Данные.НайтиСтроки(Новый Структура("ЗаказПокупателя", Заказ.ЗаказПокупателя));
			Для Каждого стикер Из тзСтикеры Цикл
				строкаТЧ = ДокОбъект.Товары.Найти(стикер.ds_ИдентификаторЗаказа, "ds_ИдентификаторЗаказа");
				строкаТЧ.ds_idСтикера = стикер.ds_idСтикера;
				строкаТЧ.ds_ШтрикходСтикера = стикер.ds_ШтрикходСтикера;
			КонецЦикла;
		...
		Исключение
			...
			ПоказатьОшибку(ошибка.Описание);
			Если ТранзакцияАктивна() Тогда
				ОтменитьТранзакцию();
			КонецЕсли;
		КонецПопытки;
...
Процедура ОбновитьОстаткиВWB(ПараметрыОбмена = Неопределено) Экспорт
	...
	ВыгрузитьОстаткиВWBПоПлануОбмена(ПараметрыОбмена);
КонецПроцедуры

Функция ВыгрузитьОстаткиВWBПоПлануОбмена(ПараметрыОбмена)

	Если Не ЗначениеЗаполнено(ПараметрыОбмена.УзелОбмена) Тогда
		ПоказатьОшибку("Узел обмена не указан в настройках. Обновление остатков невозможно");
		Возврат Ложь;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(КонтрагентДляПривязкиТоваров) Тогда
		ПоказатьОшибку("В настройках не указан контрагент для привязки товаров. Обновление остатков невозможно");
		Возврат Ложь;
	КонецЕсли;

	Результат = Истина;

	ВыгрузкаИзменений = ПроверитьПолнуюВыгрузку(ПараметрыОбмена);

	СписокНоменклатуры = Новый Массив;
	Если ВыгрузкаИзменений Тогда
		УзелОбменаПоСайту = ПараметрыОбмена.УзелОбмена;

		ВыборкаИзменений = ПланыОбмена.ВыбратьИзменения(УзелОбменаПоСайту, УзелОбменаПоСайту.НомерОтправленного + 1);
		Пока ВыборкаИзменений.Следующий() Цикл
			ОбъектОбмена = ВыборкаИзменений.Получить();
			СписокНоменклатуры.Добавить(ОбъектОбмена.Ссылка);
		КонецЦикла;
	КонецЕсли;

	Запрос = ЗапросОстаткиИЦены(ВыгрузкаИзменений);
	Запрос.УстановитьПараметр("СписокНоменклатуры", СписокНоменклатуры);
	Запрос.УстановитьПараметр("КонтрагентПривязки", КонтрагентДляПривязкиТоваров);
	Выборка = Запрос.Выполнить().Выбрать();
	...
	Пока Выборка.Следующий() Цикл
		Структ = Новый Структура("barcode,warehouseId", Выборка.ШтрихкодWB, idСкладаПоставщика);

		Если Выборка.Количество = 0 Тогда
			УдаляемыеОстатки.Добавить(Структ);
		Иначе
			Структ.Вставить("stock", Выборка.Количество);
	...
	Если УдаляемыеОстатки.Количество() > 0 Тогда
		...
		Результат = Результат И ОтветПоУдаляемым.УспехЗапроса;
	КонецЕсли;
	...
	Если ОбновляемыеОстатки.Количество() > 0 Тогда
		...
		Результат = Результат И ОтветПоОбновляемым.УспехЗапроса;
	КонецЕсли;

	Если Результат И ВыгрузкаИзменений Тогда
		УзелОбъект = УзелОбменаПоСайту.ПолучитьОбъект();
		УзелОбъект.НомерОтправленного = УзелОбъект.НомерОтправленного + 1;
		УзелОбъект.Записать();

		ПланыОбмена.УдалитьРегистрациюИзменений(УзелОбменаПоСайту, УзелОбъект.НомерОтправленного);
	КонецЕсли;
...
Функция ЗапросОстаткиИЦены(ВыгрузкаИзменений)
	Запрос = Новый Запрос;
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Магазины.Ссылка КАК Магазин,
	|	ТоварыНаСкладахОстаткиИОбороты.Номенклатура КАК Номенклатура,
	|	Магазины.ds_ИдентификаторАДЗ КАК ИдентификаторАДЗ,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.ОстаткиИОбороты(НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ),
	|			КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ), ДЕНЬ,, Номенклатура.ds_УИД <> """"
	|		И &ОтборНоменклатура) КАК ТоварыНаСкладахОстаткиИОбороты
	|		ПО Магазины.СкладПродажи = ТоварыНаСкладахОстаткиИОбороты.Склад
	|ГДЕ
	|	Магазины.ds_ВыгружатьНаСайт
	...
	|	ЦеныНоменклатуры.ds_КатегорияНоменклатуры.GUID КАК Категория,
	|	ЦеныНоменклатуры.ВидЦены
	|ПОМЕСТИТЬ Цены
	|ИЗ
	|	Остатки КАК Остатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры КАК ЦеныНоменклатуры
	|		ПО Остатки.ВидЦены = ЦеныНоменклатуры.ВидЦены
	|		И Остатки.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|ГДЕ
	...
	|		СГРУППИРОВАТЬ ПО
	|			Цены.Номенклатура,
	|			Цены.ВидЦены) КАК ВложенныйЗапрос
	|		ПО Цены.Период = ВложенныйЗапрос.Период
	|		И Цены.Номенклатура = ВложенныйЗапрос.Номенклатура
	|		И Цены.ВидЦены = ВложенныйЗапрос.ВидЦены
	|СГРУППИРОВАТЬ ПО
	|	Цены.Номенклатура,
	|	Цены.Цена,
	|	Цены.Категория,
	...
	|	Цены.Категория КАК Категория,
	|	Цены.ВидЦены
	|ПОМЕСТИТЬ ЦеныБезТекущих
	|ИЗ
	|	Цены КАК Цены
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТекущиеЦены КАК ТекущиеЦены
	|		ПО Цены.Номенклатура = ТекущиеЦены.Номенклатура
	|		И Цены.Период < ТекущиеЦены.Период
	|		И Цены.ВидЦены = ТекущиеЦены.ВидЦены
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЦеныБезТекущих.Период КАК Период,
	|	ЦеныБезТекущих.Номенклатура КАК Номенклатура,
	|	ЦеныБезТекущих.Цена КАК Цена,
	|	ЦеныБезТекущих.Категория КАК Категория,
	|	ЦеныБезТекущих.ВидЦены
	|ПОМЕСТИТЬ СтарыеЦены
	|ИЗ
	|	ЦеныБезТекущих КАК ЦеныБезТекущих
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			МАКСИМУМ(ЦеныБезТекущих.Период) КАК Период,
	|			ЦеныБезТекущих.Номенклатура КАК Номенклатура,
	|			ЦеныБезТекущих.ВидЦены
	|		ИЗ
	|			ЦеныБезТекущих КАК ЦеныБезТекущих
	|		СГРУППИРОВАТЬ ПО
	|			ЦеныБезТекущих.Номенклатура,
	|			ЦеныБезТекущих.ВидЦены) КАК ВложенныйЗапрос
	|		ПО ЦеныБезТекущих.Период = ВложенныйЗапрос.Период
	|		И ЦеныБезТекущих.Номенклатура = ВложенныйЗапрос.Номенклатура
	|		И ЦеныБезТекущих.ВидЦены = ВложенныйЗапрос.ВидЦены
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ НоменклатураWB
	|ИЗ
	|	РегистрСведений.НоменклатураКонтрагентовБЭД КАК НоменклатураКонтрагентовБЭД
	|ГДЕ
	|	НоменклатураКонтрагентовБЭД.Владелец = &КонтрагентПривязки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Остатки.Количество КАК Количество,
	|	ЕСТЬNULL(СтарыеЦены.Цена, 0) КАК СтараяЦена,
	|	ЕСТЬNULL(ТекущиеЦены.Цена, 0) КАК Цена,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТекущиеЦены КАК ТекущиеЦены
	|		ПО Остатки.Номенклатура = ТекущиеЦены.Номенклатура
	|		И Остатки.ВидЦены = ТекущиеЦены.ВидЦены
	|		ЛЕВОЕ СОЕДИНЕНИЕ СтарыеЦены КАК СтарыеЦены
	|		ПО Остатки.Номенклатура = СтарыеЦены.Номенклатура
	|		И Остатки.ВидЦены = СтарыеЦены.ВидЦены
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	НоменклатураWB.ШтрихкодWB
	|ИЗ
	|	Заготовка КАК Заготовка
	|		ЛЕВОЕ СОЕДИНЕНИЕ НоменклатураWB КАК НоменклатураWB
	|		ПО Заготовка.Номенклатура = НоменклатураWB.Номенклатура";
	Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДатаСеанса());
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборНоменклатура", ?(ВыгрузкаИзменений,
		"Номенклатура В(&СписокНоменклатуры)", "ИСТИНА"));
	
		//Не подходит для WB, поскольку его АПИ не подразумевает полную перезапись всех остатков, а требует отдельным методом обнулить остатки. 
//	Если НЕ ВыгрузкаИзменений Тогда
//		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И ТоварыНаСкладахОстаткиИОбороты.КоличествоКонечныйОстаток >= 0", "И ТоварыНаСкладахОстаткиИОбороты.КоличествоКонечныйОстаток > 0");
//	КонецЕсли;
...
Функция ПолучитьСтикерыНаWB(МассивИДЗаказа)
	...
	Для Индекс = 0 По МассивИДЗаказа.ВГраница() Цикл
		...
		ПорцияМассив.Добавить(Число(МассивИДЗаказа.Получить(Индекс)));
	КонецЦикла;
...
Функция ПолучитьШтрихкодПоставкиWBv2(IdПоставки)
...
Функция СформироватьТаблицуСтикеров(тзЗаказПокупателяИКодСтроки)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
						  ...
						  |ПОМЕСТИТЬ ВТ
						  |ИЗ
						  |	&ТоварыРеализации КАК ВременнаяТаблица
						  |;
						  |
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  ...
						  |	ВТ.КодСтроки,
						  |	ЗаказПокупателяТовары.ds_ИдентификаторЗаказа,
						  |	ЗаказПокупателяТовары.ds_idСтикера,
						  |	ЗаказПокупателяТовары.ds_ШтрикходСтикера
						  |ИЗ
						  |	ВТ КАК ВТ
						  |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
						  |		ПО ВТ.ЗаказПокупателя = ЗаказПокупателяТовары.Ссылка
						  |		И ВТ.КодСтроки = ЗаказПокупателяТовары.КодСтроки
						  |ГДЕ
						  |	ЗаказПокупателяТовары.ds_ШтрикходСтикера = """"");

	Запрос.УстановитьПараметр("ТоварыРеализации", тзЗаказПокупателяИКодСтроки);

	тз = Запрос.Выполнить().Выгрузить();
	ОтветWB = ПолучитьСтикерыНаWB(тз.ВыгрузитьКолонку("ds_ИдентификаторЗаказа"));

	Если Не ОтветWB.УспехЗапроса Тогда
		Возврат Неопределено;
	КонецЕсли;

	Для Каждого стикер Из ОтветWB.Результат.data Цикл
		строкаТЗ = тз.Найти(Формат(стикер.orderId, "ЧГ="));
		строкаТЗ.ds_idСтикера = стикер.sticker.wbStickerId;
		строкаТЗ.ds_ШтрикходСтикера = стикер.sticker.wbStickerEncoded;
	КонецЦикла;

	Возврат тз;
КонецФункции

Функция ПроверитьПолнуюВыгрузку(ПараметрыОбмена)

	Если ПараметрыОбмена.Свойство("ПолнаяВыгрузка") И ПараметрыОбмена.ПолнаяВыгрузка Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УспешныеОбменыССайтом.НастройкаОбменаССайтом КАК НастройкаОбменаССайтом
	|ИЗ
	|	РегистрСведений.УспешныеОбменыССайтом КАК УспешныеОбменыССайтом
	|ГДЕ
	|	УспешныеОбменыССайтом.НастройкаОбменаССайтом = &НастройкаОбменаССайтом
	|	И УспешныеОбменыССайтом.ДатаОкончания > &ДатаОкончания
	|	И УспешныеОбменыССайтом.ДействиеПриОбмене = &ДействиеПриОбмене";

	Запрос.УстановитьПараметр("ДатаОкончания", НачалоДня(ТекущаяДатаСеанса()));
	Запрос.УстановитьПараметр("НастройкаОбменаССайтом", ПараметрыОбмена.УзелОбмена);
	Запрос.УстановитьПараметр("ДействиеПриОбмене", Перечисления.ДействияПриОбменеССайтом.ВыгрузкаДанных);
...
#Область ПодсистемаПечатиЭтикеток

Процедура Печать(КоллекцияПечатныхФорм, ДанныеПечати) Экспорт

	УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, "ПечатьЭтикетокВБ", НСтр("ru = 'Печать этикеток ВБ'"),
		СформироватьЭтикеткиСборочныхЗаданий(ДанныеПечати), , );

КонецПроцедуры

Функция СформироватьЭтикеткиСборочныхЗаданий(Параметры)
	...
	// Макет "Эталон" используется для корректного масштабирования картинки шк 
	...
	//Получить макет
		Макет = ПолучитьМакет("ЭтикеткаСборкаВБ");
		...
		Шапка.Параметры.НаименованиеТовара = СтрДанные.Номенклатура.НаименованиеПолное;
		ТабДок.Вывести(Шапка);

		ОбластьШтрихкод = Макет.ПолучитьОбласть("Штрихкод");
		ОбластьШтрихкод.Параметры.ШтрихкодЗначение = СтрДанные.ШтрихкодЧитабельный;

		ПараметрыШтрихкода = Новый Структура;
		ПараметрыШтрихкода.Вставить("Ширина", ОбластьШтрихкод.Рисунки.КартинкаШтрихкода.Ширина
			/ КоличествоМиллиметровВПикселе);
		ПараметрыШтрихкода.Вставить("Высота", ОбластьШтрихкод.Рисунки.КартинкаШтрихкода.Высота
			/ КоличествоМиллиметровВПикселе);
		ПараметрыШтрихкода.Вставить("ОтображатьТекст", Ложь);
		ПараметрыШтрихкода.Вставить("ТипКода", 4);
		ПараметрыШтрихкода.Вставить("Штрихкод", СтрДанные.Штрихкод);

		РезультатГенерацииШк = ГенерацияШтрихкода.ИзображениеШтрихкода(ПараметрыШтрихкода);
		ОбластьШтрихкод.Рисунки.КартинкаШтрихкода.Картинка = РезультатГенерацииШк.Картинка;

		ТабДок.Вывести(ОбластьШтрихкод);

		ТабДок.Область(НомерСтрокиНачало, , ТабДок.ВысотаТаблицы, ).Имя = СтрДанные.ШтрихкодЧитабельный;
